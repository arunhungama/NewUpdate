    ...
    fun appsflyerSetUp() {
        ...
        val conversionListener: AppsFlyerConversionListener = object : AppsFlyerConversionListener {
            override fun onConversionDataSuccess(conversionDataMap: Map<String, Any>) {
                for (attrName in conversionDataMap.keys){
                    setLog(TAG, "Conversion attribute: " + attrName + " = " + conversionDataMap[attrName])
                ...
                if (status == "Non-organic") {
                    if (Objects.requireNonNull(conversionDataMap["is_first_launch"]).toString() == "true") {
                        setLog(TAG, "AppsFlyerLib Conversion: First Launch")
                    } else {
                        setLog(TAG, "AppsFlyerLib Conversion: Not First Launch")
                    }
                } else {
                    setLog(TAG, "AppsFlyerLib Conversion: This is an organic install.")
                }
                conversionData = conversionDataMap
            }
            override fun onConversionDataFail(errorMessage: String) {
                setLog(TAG, "AppsFlyerLib error getting conversion data: $errorMessage")
            }
            override fun onAppOpenAttribution(attributionData: Map<String, String>) {
                setLog(TAG, "onAppOpenAttribution: This is fake call.")
            }
            override fun onAttributionFailure(errorMessage: String) {
                setLog(TAG, "AppsFlyerLib error onAttributionFailure : $errorMessage")
            }
        }
        AppsFlyerLib.getInstance().init(APPSFLYER_APP_ID, conversionListener,
            HungamaMusicApp.getInstance().applicationContext).subscribeForDeepLink(DeepLinkListener { deepLinkResult ->
            val dlStatus = deepLinkResult.status
            if (dlStatus == DeepLinkResult.Status.FOUND) {
                setLog(TAG, "AppsFlyerLib Deep link found")
            }
            else if (dlStatus == DeepLinkResult.Status.NOT_FOUND) {
                setLog(TAG, "AppsFlyerLib Deep link not found")
            }
            else {
                // dlStatus == DeepLinkResult.Status.ERROR
                val dlError = deepLinkResult.error
                setLog(TAG, "AppsFlyerLib There was an error getting Deep Link data: $dlError")
            }
            val deepLinkObj = deepLinkResult.deepLink
            try {
                setLog("printDeeplink", "AppsFlyerLib The DeepLink data is: ${deepLinkObj}")
                setLog("","AppsFlyerLib af_referrer_customer_id:${deepLinkObj?.getStringValue("af_referrer_customer_id")}")
                val deeplingApss1 = deepLinkObj.getStringValue("deep_link_value").toString()
                println("appsflyerDeeplink"+ " appfsflyr$deeplingApss1")
                if (deeplingApss1 != "null" && deeplingApss1.isNotBlank())
                {
                    deeplink = deeplingApss1
                }
              //  deeplink = "hungama://www.hungama.com/originals?alang=gu&skiplogin=false"
                if (deeplink.isNotEmpty()) {
                    val intent = Intent(Constant.DEEPLINK)
                    LocalBroadcastManager.getInstance(HungamaMusicApp.getInstance().applicationContext).sendBroadcast(intent)
                }
                ...
                setLog(TAG, "AppsFlyerLib DeepLink data came back null")
            }
            // An example for using is_deferred
            if (deepLinkObj.isDeferred!!) {
                setLog(TAG, "AppsFlyerLib This is a deferred deep link")
            } else {
                setLog(TAG, "AppsFlyerLib This is a direct deep link")
            }

            setLog(TAG, "AppsFlyerLib DeepLink url:${deepLinkResult.deepLink?.deepLinkValue}")
        })
        ...
        AppsFlyerLib.getInstance().start(HungamaMusicApp.getInstance().applicationContext, APPSFLYER_APP_ID, object :
            ...
            override fun onError(errorCode: Int, errorDesc: String) {
                setLog("AppsFlyerLib", "Launch failed to be sent:\n" + "Error code: " + errorCode + "\n" + "Error description: " + errorDesc)
            }
        })
