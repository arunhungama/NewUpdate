...
class UserRepos (){
    ...
    private var userBookmarkidResp = MutableLiveData<Resource<BookmarkDataModel>>()

    private var homeResp = MutableLiveData<Resource<FollowModel>>()
    ...
    fun socialLogin(
        context: Context,
        url: String,
        json: String): MutableLiveData<Resource<SocialLoginRespModel>> {
        CoroutineScope(Dispatchers.IO).launch {
...
//                        }
    ...
    fun getUserFollowData(
        context: Context,
        userId: String, contentType: String
    ): MutableLiveData<Resource<FollowModel>> {
        CoroutineScope(Dispatchers.IO).launch {
            val url = WSConstants.METHOD_USER_SOCIAL_CONTENT + userId + "/$contentType/content/follow"
            ...
            DataManager.getInstance(context)
                ?.getVolleyRequest(context, url, JSONObject(), object : DataValues {
                    override fun setJsonDataResponse(response: JSONObject?) {
                        if (countryDataResp == null) {
                            homeResp = MutableLiveData<Resource<FollowModel>>()
                        }

                        try {
                            val dataModel = Gson().fromJson<FollowModel>(response.toString(), FollowModel::class.java) as FollowModel

                            homeResp.postValue(Resource.success(dataModel))
                            ...
                            hashMap.put(EventConstant.NAME_EPROPERTY, "follow")
                            hashMap.put(
                                EventConstant.RESPONSECODE_EPROPERTY,
                                EventConstant.RESPONSE_CODE_200
                            )
...
//                userBookmarkResp.postValue(Resource.error(context.getString(R.string.discover_str_2), null))
    ...
    fun getUserBookmarkedDataAll(
        context: Context
    ): MutableLiveData<Resource<BookmarkDataModel>> {
        CoroutineScope(Dispatchers.IO).launch {

            var url = WSConstants.METHOD_USER_BOOKMARK + SharedPrefHelper.getInstance()
                .getUserId() + "/1/bookmark/type?typeId=99999"
            ...
            DataManager.getInstance(context)
                ?.getVolleyRequest(context, url, JSONObject(), object : DataValues {
                    override fun setJsonDataResponse(response: JSONObject?) {
                        if (userBookmarkResp == null) {
                            userBookmarkidResp = MutableLiveData<Resource<BookmarkDataModel>>()
                        }
                        ...
                        try {
                            ...
                            userBookmarkidResp.postValue(Resource.success(dataModel))
...
//
//
//        val call: Call<LibraryAllRespModel> = quotesApi.getLibraryData(json)
//        call.enqueue(object : Callback<LibraryAllRespModel> {
//            override fun onResponse(call: Call<LibraryAllRespModel>,
//                response: Response<LibraryAllRespModel>) {
//
//                onProductData.onSuccess((response.body() as  LibraryAllRespModel))
//            }
//
//            override fun onFailure(call: Call<LibraryAllRespModel>, t: Throwable) {
...
//    }
    ...
    interface OnProductData {
        fun onSuccess(data: LibraryAllRespModel)
        fun onFailure()
    }
