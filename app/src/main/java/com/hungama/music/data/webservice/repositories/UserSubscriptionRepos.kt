...
class UserSubscriptionRepos {
    ...
    private var offlineEventListResp = MutableLiveData<Resource<OfflineEventRespModel>>()
...
//        val url = "https://ugc.api.hungama.com/v1/user/877752045/purchasehistory"
    ...
    fun getOfflineEventData(context: Context): MutableLiveData<Resource<OfflineEventRespModel>> {

        CoroutineScope(Dispatchers.IO).launch{
            val url=WSConstants.PAYTM_EVENT_INFO+"/paytm?user_id="+SharedPrefHelper.getInstance().getUserId()

            var requestTime = DateUtils.getCurrentDateTime()
            offlineEventListResp.postValue(Resource.loading(null))
            DataManager.getInstance(context)?.getVolleyRequest(context,url, JSONObject(),object : DataValues {
                override fun setJsonDataResponse(response: JSONObject?) {
                    if (offlineEventListResp == null) {
                        offlineEventListResp = MutableLiveData<Resource<OfflineEventRespModel>>()
                    }

                    try {
                        val dataModel = Gson().fromJson<OfflineEventRespModel>(
                            response.toString(),
                            OfflineEventRespModel::class.java
                        ) as OfflineEventRespModel

                        offlineEventListResp.postValue(Resource.success(dataModel))
                        ...
                        exp.printStackTrace()
                        offlineEventListResp.postValue(Resource.error(context.getString(R.string.discover_str_2), null))
                    }
                }
