                    ...
                    ?.getVolleyRequest(context, url, JSONObject(), object : DataValues {
                        override fun setJsonDataResponse(response: JSONObject?) {
                            ...
                            try {
                                ...
                                if (playableContentModel.data.body.data.url.playable.size > 0) {
                                    ...
                                    run loop@{
                                        playableContentModel.data.body.data.url.playable.forEach {
                                            ...
                                            setLog(
                                                "playbackQuality",
                                                "BaseActivity-getPlayableContentUrl-main===id-${playableContentModel.data.head.headData.id}-title-${playableContentModel.data.head.headData.title} - key-${playableContentModel.data.head.headData.misc.urlKey}--${it.key} - streamQuality-$streamQuality- isGoldUser-${BaseActivity.getIsGoldUser()}"
                                            )
                                            playableContentModel = getStreamUrl(
                                                it,
                                ...
                                setLog(
                                    "playbackQuality",
                                    "BaseActivity-getPlayableContentUrl-End-id-${playableContentModel.data.head.headData.id}-title-${playableContentModel.data.head.headData.title} - key-${playableContentModel.data.head.headData.misc.urlKey} - isGoldUser-${BaseActivity.getIsGoldUser()}"
                                )
    ...
    fun getPlayableContentList(context: Context, id:String): MutableLiveData<Resource<PlayableContentModel>> {
        CoroutineScope(Dispatchers.IO).launch {
            if (!id.isNullOrBlank()) {
                ...
                val contentType = 4
                val url =
                    WSConstants.METHOD_PLAYABLE_CONTENT + id + "/url/playable?quality=" + quality + "&contentType=" + contentType + "&certificate=widevine" + "&user=" + if (CommonUtils.isUserHasGoldSubscription()) "gold" else "free"
                HungamaMusicApp.getInstance().userStreamIDList.add(id)
