...
@OptIn(markerClass = UnstableApi.class) public class IntentUtil {
  ...
  public static void addToIntent(List<MediaItem> mediaItems, Intent intent) {
    ...
    if (mediaItems.size() == 1) {
      ...
      MediaItem.LocalConfiguration playbackProperties = checkNotNull(mediaItem.playbackProperties);
...
//                    .build());
  ...
  private static void addPlaybackPropertiesToIntent(
          MediaItem.LocalConfiguration playbackProperties, Intent intent, String extrasKeySuffix) {
    intent
