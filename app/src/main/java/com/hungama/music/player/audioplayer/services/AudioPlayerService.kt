        ...
        when (intent.action) {
            PlaybackControls.PLAY.name -> {
                ...
                CoroutineScope(Dispatchers.Main).launch {
                    ...
                    var isPause = intent.getBooleanExtra(Constant.IS_PAUSE, false)
                ...
                if (playbackStarted) {
                    ...
                    if (intent.action == PlaybackControls.ADD_TO_QUEUE.name) {
                        ...
                        if (showAddToQueueToast){
                            ...
                            CommonUtils.showToast(this, messageModel,"AudioPlayerService","PlaybackControls.ADD_TO_QUEUE")
                        }
                    }else {
                        val track = intent.getSerializableExtra(Constant.SELECTED_TRACK) as Track
                        ...
                        val messageModel = MessageModel(getString(R.string.toast_str_44), getString(R.string.toast_str_44),
                            MessageType.NEUTRAL, true)
                        CommonUtils.showToast(this, messageModel,"AudioPlayerService","onStartCommand")
                    }
                    ...
                    val isQueueTrackList = intent.getBooleanExtra(Constant.IS_QUEUE_TRACK_LIST, false)
                    if (isQueueTrackList){
                        val trackList = intent.getSerializableExtra(Constant.TRACKS_LIST) as ArrayList<Track>
                    ...
                    audioPlayer.playTracks(this@AudioPlayerService, tracksList, 0)
                    }
                }
            }
        }
...
//        audioPlayer.notificationCleanup()
    ...
    fun initializeAds(context: Context?, companionView: ViewGroup?) {
        ...
        val container =
            context?.let { imaService?.imaVideoAdPlayer?.let { it1 ->
                ImaSdkFactory.createAudioAdDisplayContainer(it,
                    it1
                )
            } }
...
/*                                    ContextCompat.startForegroundService(
                                    ...
                                    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.TIRAMISU) {
                                        startForeground(PLAYBACK_NOTIFICATION_ID, notification, ServiceInfo.FOREGROUND_SERVICE_TYPE_MEDIA_PLAYBACK)
                                    }
                                    else
                                        startForeground(PLAYBACK_NOTIFICATION_ID, notification)
                            ...
                            override fun onCustomAction(
                                ...
                                /*if(CommonUtils.checkGoldMusicPodcast()) {
                                    onNotificationButtonClickAction(this@AudioPlayerService, action)
                                }else if(action.equals(Constant.CUSTOM_ACTION_PLAY)){
                                    onNotificationButtonClickAction(this@AudioPlayerService, action)
                                }
                                else{
                                    baseActivity?.pausePlayer()
            ...
            if(parentMediaId.equals(HUNGAMA_ROOT,true)){
                ...
                return Futures.immediateFuture(
                    LibraryResult.ofItemList(
                        HungamaChannelHelper.getUserRecentPlay(
                            this@AudioPlayerService,
                            parentMediaId
                        )!!,
                ...
                return Futures.immediateFuture(
                    LibraryResult.ofItemList(
                        HungamaChannelHelper.getPodcastBucketList(
                            this@AudioPlayerService,
                            parentMediaId
                        ),
                        LibraryParams.Builder().build()))
                ...
                return HungamaChannelHelper.getDiscoverBucketList(
                    this@AudioPlayerService,
                    parentMediaId
                )
                ...
                return Futures.immediateFuture(
                    LibraryResult.ofItemList(
                        HungamaChannelHelper.getHeaderWiseListing(
                            this@AudioPlayerService,
                            parentMediaId
                        )!!,
                        LibraryParams.Builder().build()))
                ...
                return Futures.immediateFuture(
                    LibraryResult.ofItemList(
                        HungamaChannelHelper.getFavPlaylistList(
                            this@AudioPlayerService,
                            parentMediaId
                        )!!,
                        LibraryParams.Builder().build()))
                ...
                return Futures.immediateFuture(
                    LibraryResult.ofItemList(
                        HungamaChannelHelper.getFavPodcastList(
                            this@AudioPlayerService,
                            parentMediaId
                        )!!,
                        LibraryParams.Builder().build()))
                ...
                return Futures.immediateFuture(
                    LibraryResult.ofItemList(
                        HungamaChannelHelper.getFavArtistList(
                            this@AudioPlayerService,
                            parentMediaId
                        )!!,
                        LibraryParams.Builder().build()))
...
//            }
        ...
        override fun onPlayerCommandRequest(
            ...
            if (playerCommand == Player.COMMAND_SEEK_TO_NEXT){
                ...
                return SessionError.ERROR_SESSION_SETUP_REQUIRED
            }else if (playerCommand == Player.COMMAND_SEEK_TO_PREVIOUS){
                if(CommonUtils.checkGoldMusicPodcast())
                baseActivity?.playPreviousSong()
                return SessionError.ERROR_SESSION_SETUP_REQUIRED

            } else if (playerCommand == Player.COMMAND_PLAY_PAUSE){
                if(!CommonUtils.checkGoldMusicPodcast())
                {
                    baseActivity?.pausePlayer()
                }
                ...
                {
                    if (baseActivity?.currentPlayer?.isPlaying == true)
                        baseActivity?.pausePlayer()
                    else
                        baseActivity?.playPlayer()
                }
...
//        }
    ...
    override fun startTrackPlayback(
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            ...
            HungamaMusicApp.getInstance().applicationContext.startService(intent)
        }
