...
class VideoPlayerActivity : AppCompatActivity(), DownloadTracker.Listener,
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        registerReceiver(broadcastReceiver, IntentFilter(ACTION_PIP_CONTROL),Context.RECEIVER_NOT_EXPORTED)
        else
        ...
        if (b != null) {
            ...
            currentDuration= b.getLong(Constant.CURRNT_PLAYER_POSIITON, 0L)

            if (b.containsKey(Constant.Trailer_url) && b.containsKey(Constant.Trailer_url))
                Trailer_url = b.getString(Constant.Trailer_url, "")

            if (b.containsKey(Constant.heading) && b.containsKey(Constant.heading))
                pass_title = b.getString(Constant.heading, "")

/*            if (b.containsKey(Constant.recent_season) && b.containsKey(Constant.recent_season))
                recent_season = b.getString(Constant.recent_season, "")*/

/*            if (b.containsKey(Constant.recent_episode) && b.containsKey(Constant.recent_episode))
                recent_episode = b.getString(Constant.recent_episode, "")*/

            if(Trailer_url.isNotEmpty()) {
              video_play_tailer(Trailer_url)
            }

            if (b.containsKey(Constant.IS_MUTE) && b.containsKey(Constant.IS_MUTE))
                isMute = b.getBoolean(Constant.IS_MUTE, false)

            if (isOriginal) original_episode = b.getString(Constant.original_episode, "")
        ...
        val intent1 = Intent(Constant.TVSHOW)
        intent1.putExtra(Constant.MUSIC_VIDEO_STATUS,Constant.PAUSE)
        HungamaMusicApp.hungamaMusicApp?.applicationContext?.let {
            LocalBroadcastManager.getInstance(it).sendBroadcast(intent1)
        }
    ...
    private fun startPlayerServer(item: ArrayList<PlayableContentModel>, playableItemPosition: Int) {
        ...
        if (CommonUtils.userCanDownloadContent(this, mainFullScreeen, dpm, this,Constant.drawer_svod_purchase,
                CommonUtils.checkEnableFlag(CommonUtils.getFirebaseConfigAdsData().drawerSvodPurchase))){
    ...
    private fun initializePlayer() {
        try {
            if (mBound && mService!=null&&mService?.playerInstance!=null) {
                ...
                if (Trailer_url.isNotEmpty())
                {
                    var b = intent.getBundleExtra(Constant.BUNDLE_KEY)

                currentDuration?.let { mPlayer?.seekTo(it) }
                }
...
//        initializePlayer()
    ...
    fun stopPlayerBGService(currentPlayerPosition:Int){
        ...
        if (Trailer_url.isEmpty() && songsList.isNotEmpty())
    ...
    private fun callUserStreamUpdate() {
        if (mPlayer != null && mPlayer?.currentPosition != null) {
            try {
                ...
                param.put(
                    "totalDuration",
                    "" + TimeUnit.MILLISECONDS.toSeconds(mPlayer?.duration!!)
                )
                //param.put("typeId",""+ ""+Constant.PLAYABLE_CONTENT_VIDEO_ID)
                if (content_type == Constant.CONTENT_TV_SHOW) {
                    if (isOriginal) {
                        ...
                        param.put(
                            "typeId",
                            "" + "" + originalSeasonList?.get(mPlayer?.currentWindowIndex!!)?.data?.type
                        )
                        typeId = originalSeasonList.get(mPlayer?.currentWindowIndex!!)?.data?.type!!
                    } else {
                        param.put(
                            "typeId",
                            "" + "" + seasonList.get(0).data?.misc?.tracks?.get(mPlayer?.currentWindowIndex!!)?.data?.type
                        )
                        typeId =
                            seasonList.get(0).data?.misc?.tracks?.get(mPlayer?.currentWindowIndex!!)?.data?.type!!
                    }

                    if (Trailer_url == "" && seasonList.isNotEmpty()) {
                        remark = "S${seasonList.get(0).data.misc.tracks.get(mPlayer?.currentWindowIndex!!).data.season.toInt()}" + " • EP${
                            String.format(
                                "%02d",
                                seasonList.get(0).data.misc.tracks.get(mPlayer?.currentWindowIndex!!).data.episode.toInt()
                            )
                        }"
                    }

                    val s_split = original_episode.split(" • ")

                    if (remark.isEmpty()){
                        remark = "${s_split.get(0)} • ${s_split.get(1)}".replace("S0","S")
                    }
                ...
                videoListViewModel = ViewModelProvider(this).get(VideoViewModel::class.java)
                 Log.d("sarvesh",param.toString())
...
/*                videoListViewModel?.updateContentValidity(
                    this@VideoPlayerActivity,
                    param
                )*/

                val intent1 = Intent(Constant.TVSHOW)
                intent1.putExtra(Constant.MUSIC_VIDEO_STATUS,Constant.Redirection)
                intent1.putExtra(Constant.Redirection,remark)
                HungamaMusicApp.hungamaMusicApp?.applicationContext?.let {
                    LocalBroadcastManager.getInstance(it).sendBroadcast(intent1)
                }
                ...
                setLog(
                    "VideoPlayerContent",
                    "VideoPlayerActivity-playedDuration-${mPlayer?.currentPosition}"
                )
                HungamaMusicApp.getInstance().userStreamList.put(
                    contentId.toString(),
                    TimeUnit.MILLISECONDS.toSeconds(mPlayer?.currentPosition!!)
                )
                ...
                if (!isStreamEventAnalyticsCalled_AF.equals(contentId) && playedDuration > 0) {
                    ...
                    if (playedDuration > 30) {
                        ...
                        if (SharedPrefHelper.getInstance()
                                .get(PrefConstant.FIRST_STREAM_VIDEO, true)
                        ) {
                            if (contentId != null && !TextUtils.isEmpty(contentId)) {
                                ...
                                AppsFlyerLib.getInstance().logEvent(
                                    ...
                                    EventConstant.AF_FIRST_STREAM_VIDEO,
                                    eventValue
                                )

                                setLog(
                                    "TAG",
                                    "AppsFlyerLib eventName:first_stream eventProperties:$eventValue"
                                )
                                SharedPrefHelper.getInstance()
                                    .save(PrefConstant.FIRST_STREAM_VIDEO, false)
                            }
                        }
                        ...
                        eventValue.put(
                            ...
                            Utils.getContentTypeName("" + typeId)
                        )
...
/*                    val mappedTrackInfo: MappingTrackSelector.MappedTrackInfo? = trackSelector?.currentMappedTrackInfo
    ...
    private fun getVideoBandwidth() {
        ...
        for (i in 0 until _arrayOfBitrate.size) {
            try {
                ...
                setLog("aghasfh", "getNewBitrateText: $bandWidth")
                ...
                if (bandWidth > 950000 && bandWidth <= 2200000) {
                    if (!_arrayOfBandwidth.containsKey(getString(R.string.quality_2))) {
                        _arrayOfBandwidth.put(getString(R.string.quality_2), bandWidth)
                    }
                ...
                if (bandWidth > 2200000) {
                    if (!_arrayOfBandwidth.containsKey(getString(R.string.movie_str_10))) {
                        _arrayOfBandwidth.put(getString(R.string.movie_str_10), bandWidth)
                    }
                    setLog("bandWidth", "Best $bandWidth")
                }
                if (bandWidth > 3500000) {
                    if (!_arrayOfBandwidth.containsKey(getString(R.string.quality_5))) {
                        _arrayOfBandwidth.put(getString(R.string.quality_5), bandWidth)
                    }
                    setLog("bandWidth", "Best $bandWidth")
                }
    ...
    private fun handleFullScreenEnterExit() {
        ...
        if (orientation == Surface.ROTATION_90 || orientation == Surface.ROTATION_270) {
            val dpValue = resources.getDimensionPixelSize(R.dimen._200sdp)
    ...
    override fun onBackPressed(){
        ...
        {
            val intent1 = Intent(Constant.TVSHOW)
            intent1.putExtra(Constant.MUSIC_VIDEO_STATUS,Constant.SEEK)

            intent1.putExtra(Constant.SEEK, mPlayer?.currentPosition)
            HungamaMusicApp.hungamaMusicApp?.applicationContext?.let {
          ...
          /*  val intent = Intent(Constant.TVSHOW).setAction(Constant.SEEK)
            intent.putExtra(Constant.SEEK, mPlayer?.currentPosition)
            LocalBroadcastManager.getInstance(this).sendBroadcast(intent)
            finish()*/
    ...
    fun playPause() {
        if (mPlayer != null) {
            if (isPlaying()) {
                ...
                val intent1 = Intent(Constant.TVSHOW)
                intent1.putExtra(Constant.MUSIC_VIDEO_STATUS,Constant.MUTE)
                HungamaMusicApp.hungamaMusicApp?.applicationContext?.let {
                    LocalBroadcastManager.getInstance(
                        it
                    ).sendBroadcast(intent1)
...
//               startDownloadSong()
    ...
    private fun setUpVideoListViewModel() {
        ...
        if (ConnectionUtil(this).isOnline) {
            ...
            CommonUtils.showToast(this, messageModel,Constant.screen_name,"setUpVideoListViewModel")
        }
    }
    ...
    private fun saveSetting(videoQuality: VideoQuality) {
        if (ConnectionUtil(this).isOnline) {
            ...
            try {
                ...
                emailSettingJson.put("autoPlay", SharedPrefHelper.getInstance().getVideoAutoPlay())
                emailSettingJson.put("streaming_quality", videoQuality.title)
            ...
            CommonUtils.showToast(this, messageModel,"VideoPlayBackSetting","saveSetting")
        }
    }

    override fun onVideoResolutionClick(videoQuality: VideoQuality) {
        try {
            saveSetting(videoQuality)
            val quality = Quality.getQualityByName(videoQuality.title.toString())
            setLog("TAG", "onVideoResolutionClick:" + quality)
...
// We remove the padding of the scrubbing, if you have a custom size juste use dimen to calculate this
    ...
    override fun dispatchKeyEvent(event: KeyEvent?): Boolean {
        ...
        if (audioManager?.getStreamMaxVolume(AudioManager.STREAM_MUSIC) != null)
        {
            volume_slider?.max
        }
        ...
        return when (keyCode) {
            KeyEvent.KEYCODE_VOLUME_UP -> {
                if (action == KeyEvent.ACTION_DOWN) {
                    ...
                    setLog("ahgksav", "${volume_slider.progress}")
                    if (isMute){
                        isMute = false
                        volIcon.setImageResource(R.drawable.volume)
                    }
    ...
    private fun setUpPlayableContentListViewModel(id:String) {
        ...
        if (ConnectionUtil(this).isOnline) {
            ...
            val messageModel = MessageModel(getString(R.string.toast_str_35), getString(R.string.toast_message_5),MessageType.NEGATIVE, true)
            CommonUtils.showToast(this, messageModel,Constant.screen_name,"setUpPlayableContentListViewModel")
        }
    }
    ...
    fun setVidelList(
        ...
        if (playableContentModel != null){
            ...
            if(playableItem?.get(position)?.data?.type != null)
            {
                track.data.head.headData.type = playableItem?.get(position)?.data?.type!!
            }
        }
    ...
    private fun callStreamEventAnalytics(track: PlayableContentModel, isStream:Boolean, duration:String) {
        try {
            ...
            if(originalSeasonList.isNotEmpty() && original_episode.contains(" • ")){
                var s_split = original_episode.split(" • ").get(1).split("EP").get(1)
                hashMap.put(EventConstant.EPISODENUMBER_EPROPERTY,""+s_split.toString())
            }
            ...
            if(CommonUtils.isUserHasGoldSubscription()){
                hashMap.put(EventConstant.streamed_by_paid_user, "yes")
            }else hashMap.put(EventConstant.streamed_by_paid_user, "no")
    ...
    private fun registerBroadcastReceiver() {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.TIRAMISU)
        applicationContext.registerReceiver(screenOnOffReceiver, theFilter,Context.RECEIVER_NOT_EXPORTED)
        else
...
//                playPlayer()
    ...
    private fun playNextVideo(){
        if (!TextUtils.isEmpty(songsList?.get(playIndex)?.data?.head?.headData?.id!!)) {
            ...
            if (CommonUtils.userCanDownloadContent(this, mainFullScreeen, dpm, this,Constant.drawer_svod_tvshow_episode,
                    CommonUtils.checkEnableFlag(CommonUtils.getFirebaseConfigAdsData().drawerSvodTvshowEpisode))){
            ...
            if (intent != null) {
                if (intent.hasExtra("EVENT")) {
                    ...
                    if ((Build.VERSION.SDK_INT >= Build.VERSION_CODES.O)
                        ...
                        if (mPlayer!=null && this@VideoPlayerActivity.isAvailable())
                        stopPlayerBGService(mPlayer!!.currentWindowIndex)
                        finishAndRemoveTask()
                    } else {
                        finish()
    ...
    fun video_play_tailer(url: String) {
        val track = PlayableContentModel()

         track.data?.head?.headData?.id = "0"
         track.data?.head?.headData?.title = ""
         track?.data?.head?.headData?.misc?.url = url
         track?.data?.head?.headData?.misc?.downloadLink?.drm?.token = ""
         track?.data?.head?.headData?.misc?.movierights = listOf()
         track.data?.head?.headData?.image = ""
         songsList.clear()
