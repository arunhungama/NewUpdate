...
import com.hungama.music.utils.Constant.DeepLink_Payment
...
abstract class BaseFragment : Fragment(), View.OnClickListener,
    ...
    protected var ivGoldLogo: AppCompatImageView? = null
...
//        ivLogo = view.findViewById(R.id.ivLogo)
        ivGoldLogo = view.findViewById(R.id.dash_gold)
        ...
        if(tvCount!=null){
            ...
            val newCardCount:String = MoEInboxHelper.getInstance().getUnClickedMessagesCount(requireContext())?.count.toString()

            setLog(TAG, "onViewCreated newCardCount:${newCardCount}")
            if(newCardCount!=null && !TextUtils.isEmpty(newCardCount.toString())){
                tvCount?.text = newCardCount
        ...
        ivLogo?.setOnClickListener {
            if (logoClickCount > 5){
                ...
                CommonUtils.showToast(requireContext(), messageModel,"BaseFragment","onViewCreated")
                logoClickCount = 0
            }else{
                logoClickCount += 1
...
//        setLog("FragmentName",nextFragment.getN)
    ...
    fun sendArtworkTappedEvent(
        ...
        CoroutineScope(Dispatchers.IO).launch {
            ...
            Constant.title_heading = parent.heading.toString()
            ...
            var title = parent.items?.get(childPosition)?.data?.title!!
            ...
            if (feature.equals("offline_event")) {
                var eventName = parent.items?.get(childPosition)?.data?.eventName
                hashMap.put(EventConstant.EVENTNAME,eventName.toString())
                hashMap.put(EventConstant.EVENTID,newContentId)
                hashMap.put(EventConstant.topNav,MainActivity.headerItemName)
            }
    ...
    fun onItemDetailPageRedirection(parent: RowsItem, parentPosition: Int, childPosition: Int, parentTitle:String) {
        ...
        if (!TextUtils.isEmpty(deeplink_url)) {
        ...
        else if (type.equals("112", true) || type.equals("115", true)) {
            ...
            addFragment(R.id.fl_container, this, audioBookDetailsFragment, false)
...
//            val playlistDetailFragment = PlaylistDetailFragmentDynamic(varient)
            ...
            addFragment(R.id.fl_container, this, paytmInsiderDetailsFragment, false)
        }
...
//                bundle.putString(Constant.EXTRA_DEEPLINK, parent.items?.get(childPosition)?.data?.deepLink)
    ...
    fun onItemDetailPageRedirectionItype50(parent: RowsItem, parentPosition: Int, childPosition: Int, parentTitle:String) {
        ...
        val title = parent?.items?.get(childPosition)?.data?.title
        val varientType = parent?.items?.get(childPosition)?.data?.variant
        val external = parent?.items?.get(childPosition)?.data?.external
        ...
        if(external?.contains("true") == true){
            val url = share
            val i = Intent(Intent.ACTION_VIEW)
            i.data = Uri.parse(url)
            activity?.startActivity(i)
            return
        }else if (type.equals("35", true)) {
            if (ConnectionUtil(requireContext()).isOnline) {
                ...
                intent.putExtra(DeepLink_Payment, genrtedURL)
...
//                bundle.putString(Constant.EXTRA_DEEPLINK, parent.items?.get(childPosition)?.data?.deepLink)
    ...
    fun callPageScrolledEvent(
        ...
        CoroutineScope(Dispatchers.IO).launch {
            ...
            dataMap.put(EventConstant.SOURCE, source)

            if(fromBucket.contains("good",true)){
            ...
            if(toBucket.contains("good",true)){
    ...
    fun commonThreeDotMenuItemSetup(
        ...
        track:PlaylistModel.Data.Body.Row.Data.Misc.Track? =  null,
        forAdapter:Int = 0
    ) {
        this.onMenuItemClicked = onMenuItemClicked
        commonThreeDotsMenuFragment = CommonThreeDotsMenuFragment.newInstance(getThreeDotMenuItemsData(detailPageId, isFavorite), this, track, forAdapter)
        setLog(
            TAG,
    ...
    override fun onThreeDotMenuItemClick(
        ...
        val movieRights = commonThreeDotsMenuItemModel.movieRights
        val forAdapter = commonThreeDotsMenuItemModel.forAdapter

        if (forAdapter != 2){
        if (menuItemId == 3 && !CommonUtils.isUserHasGoldSubscription() && movieRights?.contains("AMOD") != true){

            CommonUtils.openSubscriptionDialogPopupNew(requireContext())
            return
        }
        }

        if (menuItemId ==12 && !CommonUtils.isUserHasGoldSubscription() && movieRights?.contains("AMOD") != true){
            CommonUtils.openSubscriptionDialogPopupNew(requireContext())
            return
        }
...
//        setLog(TAG, "onThreeDotMenuItemClick title: ${commonThreeDotsMenuItemModel?.title}")
        ...
        else if (detailPageId == AUDIO_BOOK_STORY_DETAIL_PAGE) {
            val podcastRespModel = AudioBookDetailsFragment.podcastRespModel
            if (podcastRespModel != null) {
                if (!TextUtils.isEmpty(podcastRespModel.data?.head?.data?.title)){
                    headTitle = podcastRespModel.data.head.data.title
                }
        ...
        else if (detailPageId == AUDIO_BOOK_STORY_DETAIL_ADAPTER) {
            val podcastRespModel = AudioBookDetailsFragment.podcastRespModel
            if (podcastRespModel != null) {
                if (!TextUtils.isEmpty(podcastRespModel.data?.head?.data?.title)){
                    headTitle = podcastRespModel.data.head.data.title
    ...
    private fun shareStoryThreeDotMenuItem(commonThreeDotsMenuItemModel: CommonThreeDotsMenuItemModel) {
        ...
        else if(commonThreeDotsMenuItemModel.detailPageId == AUDIO_BOOK_STORY_DETAIL_PAGE){
            val title = AudioBookDetailsFragment.podcastRespModel?.data?.head?.data?.title
            val subtitle = "By"+" "+AudioBookDetailsFragment.podcastRespModel?.data?.head?.data?.subtitle
            val contentURL = AudioBookDetailsFragment.podcastRespModel?.data?.head?.data?.misc?.share
            val imageURL = AudioBookDetailsFragment.podcastRespModel?.data?.head?.data?.image

            setLog(TAG, "shareStoryThreeDotMenuItem:AUDIO_BOOK_STORY_DETAIL_PAGE "+title)
...
//            val subtitle = ChartDetailFragment.playlistRespModel?.data?.head?.data?.subtitle
        ...
        else if(commonThreeDotsMenuItemModel.detailPageId == AUDIO_BOOK_STORY_DETAIL_ADAPTER){
            val title = AudioBookDetailsFragment?.podcastEpisode?.data?.title
            val subtitle = AudioBookDetailsFragment?.podcastRespModel?.data?.head?.data?.title
            val contentURL = AudioBookDetailsFragment?.podcastRespModel?.data?.head?.data?.misc?.share
            val imageURL = AudioBookDetailsFragment?.podcastRespModel?.data?.head?.data?.image

            setLog(TAG, "shareStoryThreeDotMenuItem:AUDIO_BOOK_STORY_DETAIL_ADAPTER "+title)
            val track = Track()
            track.title = title
            track.subTitle = subtitle
            track.url = contentURL
...
//            threeDotMenuItemsList.add(getThreeDotMenuItem(SIMILAR_SONGS_MENU_ITEM, detailPageId))
    ...
    private fun shareThreeDotMenuItem(commonThreeDotsMenuItemModel: CommonThreeDotsMenuItemModel) {
        ...
        if (detailPageId == ALBUM_DETAIL_PAGE) {
            ...
            val shareurl = getString(R.string.music_player_str_18) + " " + AudioBookDetailsFragment?.podcastRespModel?.data?.head?.data?.misc?.share
            Utils.shareItem(requireActivity(), shareurl)
        }
        else if (detailPageId == SONG_DETAIL_PAGE) {
            var shareurl =
            ...
            var shareurl =
                getString(R.string.music_player_str_18) + " " + TvShowDetailsFragment.tvShowDetailRespModel1?.data?.head?.share
            Utils.shareItem(requireActivity(), shareurl)
        } else if (detailPageId == TVSHOW_DETAIL_ADAPTER) {
            var shareurl =
            ...
            var shareurl =
                getString(R.string.music_player_str_18) + " " + AudioBookDetailsFragment?.podcastEpisode?.data?.misc?.share
            Utils.shareItem(requireActivity(), shareurl)
        }
        else if (detailPageId == PLAYLIST_DETAIL_ADAPTER) {
            var shareurl =
    ...
    private fun addToQueueThreeDotMenuItem(commonThreeDotsMenuItemModel: CommonThreeDotsMenuItemModel) {
        ...
        if (detailPageId == ALBUM_DETAIL_PAGE) {
            ...
            if (podcastRespModel != null) {
                if (podcastRespModel?.data?.body?.rows?.get(0)?.data?.misc?.tracks?.size!! > 0) {
                    ...
                    if (trackDataList.isNotEmpty()) {
                        for (i in trackDataList.indices) {
                            if (i >= trackDataList.size)
                                break
                            if (!CommonUtils.checkMovieRight(trackDataList.get(i).movierights.toString())) {
                                trackDataList.removeAt(i)
                                isNonGold = true
                            }
                        }
                    }
                    if (!CommonUtils.isUserHasGoldSubscription() && isNonGold) {
                        CommonUtils.openSubscriptionDialogPopupNew(requireContext())
                    }
                    val intent = Intent(context, AudioPlayerService::class.java)
                    intent.apply {
        ...
        {
            val pdf = AudioBookDetailsFragment()
            val podcastRespModel = AudioBookDetailsFragment.podcastRespModel
            if (podcastRespModel != null) {
                if (podcastRespModel?.data?.body?.rows?.get(0)?.data?.misc?.tracks?.size!! > 0) {
                    trackDataList = arrayListOf()
                    ...
                    for (i in podcastRespModel?.data?.body?.rows?.get(0)?.data?.misc?.tracks?.indices!!) {
                        trackDataList = pdf.setPodcastEpisodeList(null, podcastRespModel?.data?.body?.rows?.get(0)?.data?.misc?.tracks, i)
                    }
                    if (trackDataList.isNotEmpty())
                    {
                        for (i in trackDataList.indices){
            ...
            val pdf = AudioBookDetailsFragment()
            val podcastRespModel = AudioBookDetailsFragment.podcastRespModel
            var playableItemPosition = AudioBookDetailsFragment.playableItemPosition
            if (podcastRespModel != null) {
                if (podcastRespModel?.data?.body?.rows?.get(0)?.data?.misc?.tracks?.size!! >= playableItemPosition) {
                    trackDataList = arrayListOf()
                    trackDataList = pdf.setPodcastEpisodeList(
            ...
            if (downloadedRespModel != null) {
                if (downloadedRespModel != null && downloadedRespModel?.size!! >= playableItemPosition) {
                    ...
                    if (CommonUtils.isUserHasGoldSubscription() || (downloadedRespModel.get(playableItemPosition).restrictedDownload != RestrictedDownload.RESTRICT_DOWNLOAD_CONTENT.value && playableItemPosition < freeSongLimit)) {
                        ...
                        CommonUtils.showToast(requireContext(), messageModel,"BaseFragment","addToQueueThreeDotMenuItem")
                    }else{
                        CommonUtils.openSubscriptionDialogPopup(
                            requireContext(),
                            PlanNames.SVOD.name,
    ...
    private fun playNextThreeDotMenuItem(commonThreeDotsMenuItemModel: CommonThreeDotsMenuItemModel) {
        ...
        if (detailPageId == ALBUM_DETAIL_PAGE) {
            ...
            val adf = AudioBookDetailsFragment()
            val audiobookRespModel = AudioBookDetailsFragment.podcastRespModel
            val playableItemPosition = AudioBookDetailsFragment.playableItemPosition
            if (audiobookRespModel != null) {
                if (audiobookRespModel?.data?.body?.rows?.get(0)?.data?.misc?.tracks?.size!! >= playableItemPosition) {
                    trackDataList = arrayListOf()
                    trackDataList = adf.setPodcastEpisodeList(
                        null,
                        audiobookRespModel?.data?.body?.rows?.get(0)?.data?.misc?.tracks,
                        playableItemPosition
                    )
                    if (trackDataList != null && trackDataList.size > 0) {
                        val intent = Intent(context, AudioPlayerService::class.java)
    ...
    private fun removeFromLibraryThreeDotMenuItem(commonThreeDotsMenuItemModel: CommonThreeDotsMenuItemModel) {
        if (commonThreeDotsMenuItemModel.detailPageId == FAVORITED_CONTENT_DETAIL_ADAPTER){
            ...
            if(favoritedRespModel?.data?.body?.rows?.size!! > FavoritedSongsDetailFragment.playableItemPosition) {
                val favoritedItemPosition = FavoritedSongsDetailFragment.playableItemPosition
                val contentId =
                    favoritedRespModel?.data?.body?.rows?.get(favoritedItemPosition)?.data?.id
                val typeId =
                    favoritedRespModel?.data?.body?.rows?.get(favoritedItemPosition)?.data?.type
                val isFavorite =
                    !favoritedRespModel?.data?.body?.rows?.get(favoritedItemPosition)?.data?.isFavorite!!
                setLog(
                    TAG,
                    "removeFromLibraryThreeDotMenuItem: favoritedItemPosition  ${favoritedItemPosition}"
                )
    ...
    fun addToPlaylistThreeDotMenuItem(commonThreeDotsMenuItemModel: CommonThreeDotsMenuItemModel) {
        ...
        if (detailPageId == AUDIO_BOOK_STORY_DETAIL_ADAPTER) {
            val playlistRespModel = AudioBookDetailsFragment.podcastEpisode
            val contentId = "" + playlistRespModel?.data?.id
            val typeId = "" + playlistRespModel?.data?.type
            val contentName = "" + playlistRespModel?.data?.title
            addToPlaylistMenuFragment = AddToPlaylistMenuFragment(contentId, contentName, typeId)
    ...
    fun setUpShuffledPlayableContentListViewModel(id: String) {
        ...
        if (isAdded && context != null && ConnectionUtil(context).isOnline) {
            ...
            CommonUtils.showToast(requireContext(), messageModel,"BaseFragment","setUpShuffledPlayableContentListViewModel")
        }
...
//            }
    ...
    public fun setEventModelDataAppLevel(playableContentModel: PlaylistModel.Data.Body.Row.Data, contentType: Int,pos: String="") {
                ...
                if (isFavourite) {
                   ...
                   CommonUtils.showToast(requireContext(), messageModel)*/
                    ...
                    if(noToast) {
                        ...
                        CommonUtils.showToast(requireContext(), messageModel,"BaseFragment","setAddOrRemoveFavourite")
                    }
                }
    ...
    fun getContentOrderStatus(
        ...
        if (isAdded) {
            ...
            if (ConnectionUtil(requireContext()).isOnline) {
                ...
                CommonUtils.showToast(requireContext(), messageModel,"BaseFragment","getContentOrderStatus")
            }
        }
