...
class BucketChildAdapter(
    ...
    val onMoreItemClick: BucketParentAdapter.OnMoreItemClick? = null,
    val parent : RowsItem? = null,
    ...
    companion object {
        ...
        const val ROW_ITYPE_50 = 50
        const val ROW_ITYPE_51 = 51
        const val ROW_ITYPE_52 = 52
        const val ROW_ITYPE_56 = 56
        const val ROW_ITYPE_57 = 57
        ...
        var originalLiveData: MutableLiveData<String> = MutableLiveData<String>()
    }
...
//    var list =mainList
    ...
    private inner class IType2ViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
        ...
        val llMain: LinearLayoutCompat = itemView.findViewById(R.id.llMain)
        val ivfreeStrips: ImageView = itemView.findViewById(R.id.ivfreeStrips)
        fun bind(position: Int) {
            if (!list.isNullOrEmpty() && list.size > position) {
                val list = list.get(position)?.data
                ...
                if(list.type.equals("109") || list.type.equals("110")) {
                    if (!CommonUtils.isUserHasGoldSubscription())
                        CommonUtils.checkProdcastCheck(mContext,list.misc?.movierights.toString(), ivfreeStrips)
                }
                else if(list.type.equals("115")) {
                    if (!CommonUtils.isUserHasGoldSubscription())
                        CommonUtils.checkAudioStoriesCheck(mContext,list.misc?.movierights.toString(), ivfreeStrips, null)
                }
    ...
    private inner class IType7ViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
        ...
        fun bind(position: Int) {
            if (!list.isNullOrEmpty() && list.size > position) {
                ...
                if(dataItem.type.equals("109") || dataItem.type.equals("110")) {
                    if (!CommonUtils.isUserHasGoldSubscription())
                        CommonUtils.checkProdcastCheck(
                            ctx,
                            dataItem.misc?.movierights.toString(),
                            ivfreeStrips
                        )
                }
                    ...
                    if (!CommonUtils.isUserHasGoldSubscription())
                    {
                        if(dataItem.type.equals("114"))
                        CommonUtils.checkAudioBookCheck(mContext, dataItem.misc?.movierights.toString(), ivfreeStrips, null)
                        else if(dataItem.type.equals("117"))
                            CommonUtils.checkAudioStoriesCheck(mContext, dataItem.misc?.movierights.toString(), ivfreeStrips, null)
                    }
                    else
                        ivfreeStrips.hide()
        ...
        fun bind(position: Int) {
            if (!list.isNullOrEmpty() && list.size > position) {
                ...
                llMain.setOnClickListener {
                    ...
                    if (!CommonUtils.isUserHasGoldSubscription()){
                        CommonUtils.openSubscriptionDialogPopupNew(ctx)
...
//                    tvSubTitle2.visibility = View.GONE
                ...
                if(list.type.equals("109") || list.type.equals("110")) {
                    if(!CommonUtils.isUserHasGoldSubscription())
                    CommonUtils.checkProdcastCheck(ctx,list.misc?.movierights.toString(), ivfreeStrips)
                }
    ...
    private inner class IType50ViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
        var tvTitle: TextView = itemView.findViewById(R.id.tvTitle)
        val tvRating: TextView = itemView.findViewById(R.id.tvRating)
        val ivUserImage: ImageView = itemView.findViewById(R.id.ivUserImage)
        val llMain: LinearLayoutCompat = itemView.findViewById(R.id.llMain)
        fun bind(position: Int) {
            if (!list.isNullOrEmpty() && list.size > position) {
                ...
                var height = 0.0
                if (!TextUtils.isEmpty(list?.title)) {
                    tvTitle.text = CommonUtils.getValueFromCurrentLang(list?.title.toString())
                    tvTitle.visibility = View.VISIBLE
                } else {
                    tvTitle.visibility = View.GONE
                }

/*                if (!TextUtils.isEmpty(rowsItem.items?.get(position)?.data?.secondaryCta) || rowsItem.items?.get(position)?.data?.secondaryCta != null) {
                ...
                if (list?.image != null) {/*
                    ...
                    params2.width = itemWidth.toInt()
                    ivUserImage.requestLayout()*/
                    ImageLoader.loadImage(ctx, ivUserImage, list.image, R.drawable.ic_game_placeholder)
                }
...
/*                if (list?.attributeGameRating?.isEmpty() == false) {
    ...
    private inner class OldIType52ViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
    ...
    private inner class IType52ViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
        ...
        fun bind(position: Int) {
            if (!list.isNullOrEmpty() && list.size > position) {
                ...
                if (!TextUtils.isEmpty(list?.startDate)) {
                    tvDay.text = com.hungama.music.utils.DateUtils.convertDate(
                        com.hungama.music.utils.DateUtils.DATE_FORMAT_YYYY_MM_DD_HH_MM_WITH_T,
                        com.hungama.music.utils.DateUtils.DATE_FORMAT_DD,
                        list?.startDate
    ...
    private inner class IType56ViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {

        val tvTitle: TextView = itemView.findViewById(R.id.tvTitle)
        val tvSubTitle: TextView = itemView.findViewById(R.id.tvSubTitle)
        val ivUserImage: ImageView = itemView.findViewById(R.id.ivUserImage)
        val ivbg1: ImageView = itemView.findViewById(R.id.ivbg1)
        val ivbg2: ImageView = itemView.findViewById(R.id.ivbg2)
        val ivbg3: ImageView = itemView.findViewById(R.id.ivbg3)
        val rlMain: RelativeLayout = itemView.findViewById(R.id.llMain)
        val tvDuration: TextView = itemView.findViewById(R.id.tvDuration)
        val ivfreeStrips: ImageView = itemView.findViewById(R.id.ivfreeStrips)
        val blurView: CustomBlurView = itemView.findViewById(R.id.blurView)
        fun bind(position: Int) {
            if (!list.isNullOrEmpty() && list.size > position) {
                val list = list.get(position)?.data
                    if (list?.image != null) {
                        setLog("lagfssvsg", "56 ${list?.misc?.movierights}")
                        if (!TextUtils.isEmpty(list.color1) && !TextUtils.isEmpty(list.color3) && !TextUtils.isEmpty(list.color3)) {
                            ivbg1.setColorFilter(Color.parseColor(list.color1!!))
                            ivbg2.setColorFilter(Color.parseColor(list.color2!!))
                            ivbg3.setColorFilter(Color.parseColor(list.color3!!))
                        }
                        else{
                            CommonUtils.setArtImageDarkBg1(list.image, ivbg1, ivbg2, ivbg3)
                        }
                        val params: ViewGroup.LayoutParams = rlMain.layoutParams
                        params.height = itemHeight.toInt()
                        params.width = (itemWidth + ctx.resources.getDimensionPixelSize(R.dimen.dimen_15).toDouble()).toInt()
                        rlMain.requestLayout()
                        val params2: ViewGroup.LayoutParams = ivUserImage.layoutParams
                        params2.height = imageHeightByAspectRatio.toInt()
                        params2.width = (itemWidth - ctx.resources.getDimensionPixelSize(R.dimen.dimen_9).toDouble()).toInt()
                        ivUserImage.requestLayout()

                        val params3: ViewGroup.LayoutParams = ivbg1.layoutParams
                        params3.height = imageHeightByAspectRatio.toInt()
                        params3.width = (itemWidth - ctx.resources.getDimensionPixelSize(R.dimen.dimen_4).toDouble()).toInt()
                        ivbg1.requestLayout()

                        val params4: ViewGroup.LayoutParams = ivbg2.layoutParams
                        params4.height = imageHeightByAspectRatio.toInt()
                        params4.width = (itemWidth + ctx.resources.getDimensionPixelSize(R.dimen.dimen_1).toDouble()).toInt()
                        ivbg2.requestLayout()

                        val params5: ViewGroup.LayoutParams = ivbg3.layoutParams
                        params5.height = imageHeightByAspectRatio.toInt()
                        params5.width = (itemWidth + ctx.resources.getDimensionPixelSize(R.dimen.dimen_6).toDouble()).toInt()
                        ivbg3.requestLayout()
                        ImageLoader.loadImage(ctx, ivUserImage, list.image, R.drawable.bg_gradient_placeholder)
                    }
                if (!CommonUtils.isUserHasGoldSubscription())
                    CommonUtils.checkAudioBookCheck(mContext,list?.misc?.movierights.toString(), ivfreeStrips, rlMain)
                else
                    ivfreeStrips.hide()
                var totalDuration = "00:00"
                if(!TextUtils.isEmpty(list?.duration)) {
                    val duration = list?.duration?.toInt()
                    if (duration != null) {
                        if (duration>0){
                    ...
                    tvDuration.text = totalDuration
                if (!TextUtils.isEmpty(list?.title)) {
                    tvTitle.text = list?.title.toString()
                    tvTitle.visibility = View.VISIBLE
                } else {
                    tvTitle.visibility = View.INVISIBLE
                }
                ...
                tvDuration.viewTreeObserver.addOnPreDrawListener(object :
                    ViewTreeObserver.OnPreDrawListener {
                    override fun onPreDraw(): Boolean {
                        tvDuration.viewTreeObserver.removeOnPreDrawListener(this)
                        val params: ViewGroup.LayoutParams = blurView.layoutParams
                        params.height = tvDuration.height + ctx.resources.getDimensionPixelSize(R.dimen.dimen_5)
                        params.width = tvDuration.width + ctx.resources.getDimensionPixelSize(R.dimen.dimen_10)
                        blurView.requestLayout()
                        return true
                    }
                })
                rlMain.setOnClickListener {
                    if (onChildItemClick != null) {
                        onChildItemClick.onUserClick(position, rlMain)
                    }
                }
            }
    ...
    private inner class IType57ViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {

        val tvDetailBtnTitle: TextView = itemView.findViewById(R.id.tvDetailBtnTitle)
        val tvTitle: TextView = itemView.findViewById(R.id.tvTitle)
        val tvSubTitle: TextView = itemView.findViewById(R.id.tvSubTitle)
        val ivUserImage: ImageView = itemView.findViewById(R.id.ivUserImage)
        val ivbg1: ImageView = itemView.findViewById(R.id.ivbg1)
        ...
        val llMain: RelativeLayout = itemView.findViewById(R.id.llMain)
        val tvDuration: TextView = itemView.findViewById(R.id.tvDuration)
        val tvDetailTitle: TextView = itemView.findViewById(R.id.tvDetailTitle)
        val lrDetails: LinearLayout = itemView.findViewById(R.id.lrDetails)
        val consArtwork: ConstraintLayout = itemView.findViewById(R.id.consArtwork)
        val blurView: CustomBlurView = itemView.findViewById(R.id.blurView)
        fun bind(position: Int) {
            if (!list.isNullOrEmpty() && list.size > position) {
                val childList = list.get(position)?.data
                setLog("lagfssvsg", "57 ${childList?.misc?.movierights}")
                    if (!childList?.image.isNullOrEmpty()) {

                        setLog("lagfssvsg", "57 ${childList?.title} ${childList?.image}")
                        lrDetails.visibility = View.GONE
                        consArtwork.visibility = View.VISIBLE
//                        list?.image?.let { CommonUtils.setArtImageDarkBg1(it, ivbg1, ivbg2, ivbg3) }
                        ivbg1.setColorFilter(Color.parseColor(childList?.color1!!))
                        ivbg2.setColorFilter(Color.parseColor(childList?.color2!!))
                        ivbg3.setColorFilter(Color.parseColor(childList?.color3!!))
                        val params: ViewGroup.LayoutParams = llMain.layoutParams
                        params.height = itemHeight.toInt()
                        params.width = (itemWidth + ctx.resources.getDimensionPixelSize(R.dimen.dimen_15).toDouble()).toInt()
                        llMain.requestLayout()
                        ...
                        val params3: ViewGroup.LayoutParams = ivbg1.layoutParams
                        params3.height = imageHeightByAspectRatio.toInt()
                        params3.width = (itemWidth + ctx.resources.getDimensionPixelSize(R.dimen.dimen_5).toDouble()).toInt()
                        ivbg1.requestLayout()

                        val params4: ViewGroup.LayoutParams = ivbg2.layoutParams
                        params4.height = imageHeightByAspectRatio.toInt()
                        params4.width = (itemWidth + ctx.resources.getDimensionPixelSize(R.dimen.dimen_10).toDouble()).toInt()
                        ivbg2.requestLayout()

                        val params5: ViewGroup.LayoutParams = ivbg3.layoutParams
                        params5.height = imageHeightByAspectRatio.toInt()
                        params5.width = (itemWidth + ctx.resources.getDimensionPixelSize(R.dimen.dimen_15).toDouble()).toInt()
                        ivbg3.requestLayout()
                        if (!CommonUtils.isUserHasGoldSubscription())
                            CommonUtils.checkAudioStoriesCheck(mContext,childList?.misc?.movierights.toString(), ivfreeStrips, llMain)
                        else
                            ivfreeStrips.hide()

                        childList?.image?.let {
                            ImageLoader.loadImage(ctx, ivUserImage,
                        ...
                        lrDetails.visibility = View.VISIBLE
                        consArtwork.visibility = View.GONE
                        ivfreeStrips.visibility = View.GONE
                        val params: ViewGroup.LayoutParams = llMain.layoutParams
                        val width: Int = (itemWidth * 1.35).toInt()
//                        params.height = imageHeightByAspectRatio.toInt()
                        params.width = width
                        llMain.requestLayout()
                        tvDetailBtnTitle.setOnClickListener{
                            val parentData = RowsItem()
                            parentData.itype = parent?.itype
                            parentData.id = parent?.id
                            parentData.more = parent?.more
                            parentData.heading = parent?.heading
                            parentData.numrow = parent?.numrow
                            parentData.items = ArrayList()

                            for (i in 1..list?.size!!-1){
                                parentData.items?.add(list?.get(i))
                            }
//                            parentData?.items?.removeAt(0)
                            setLog("printItems", "${parent?.items?.size}   ${parentData.items?.size}")

                            onMoreItemClick?.onMoreClick(parentData, parentPosition)
                        }
                    }
                if (!TextUtils.isEmpty(childList?.title)) {
                    tvTitle.text = childList?.title.toString()
                    tvTitle.visibility = View.VISIBLE
                } else {
                    tvTitle.visibility = View.INVISIBLE
                }

                if (childList?.subTitle != null) {
                    tvSubTitle.text = childList?.subTitle
                    tvSubTitle.visibility = View.VISIBLE
                } else {
                    tvSubTitle.visibility = View.INVISIBLE
                }
                var totalDuration = "00:00"
                if(!TextUtils.isEmpty(childList?.duration)) {
                    val duration = childList?.duration?.toInt()
                    if (duration != null) {
                        if (duration>0){
                            var hours = duration?.div(3600).toString()
                            var minutes = (duration?.rem(3600))?.div(60).toString()
        ...
        fun bind(position: Int) {
            if (!list.isNullOrEmpty() && list.size > position) {
                ...
                if (model?.options != null && model.options.size > 0) {
                    ...
                    if (mInAppSelfHandledModel != null) {
                        val inAppSelfHandledModel = Gson().fromJson<InAppSelfHandledModel>(
                            mInAppSelfHandledModel?.inAppCampaign?.campaign?.payload,
                ...
                //                    SharedPrefHelper.getInstance().save(InAppCallback.mInAppCampaign?.campaignId!!,InAppCallback.mInAppCampaign?.campaignId!!)
                            ...
                            MoEInAppHelper.getInstance().selfHandledClicked(ctx, mInAppSelfHandledModel?.inAppCampaign!!)
    ...
    private inner class IType47ViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
        ...
        fun bind(position: Int) {
            if (!list.isNullOrEmpty() && list.size > position) {
                ...
                originalLiveData.observe(ctx as LifecycleOwner, Observer {
                    if(!it.isNullOrEmpty()) {
...
//                videoUri = "https://hunstream.hungama.com/c/5/f60/f2e/58125461/58125461_,750,100,400,1000,1600,3000,6500,.mp4.m3u8?vjyjrH8V8iq1KfVtl8bIiLkQt-RYo_Xc_ITUZH8i7ioESjkwQGT7AQdH2Rnr4kWB6OSep5la4dcrLU4c80-JgXI6wqKTNT6NX7ja83eSH8gnoRfa5yJz4XZ7yIm2"
                    ...
                    if (!videoUri.isNullOrBlank()) {
                        ...
                        if (list.get(position)?.orignalItems?.get(0)?.isVisible == true) {
                            setLog(
                                "DiscoverOriginalkTrailer"," $position ${
                            ...
                            val track = Track()
                            track.id =
                                list.get(position)?.orignalItems?.get(0)?.data?.id?.trim()?.toLong()!!
                            ...
                            simpleExoplayer = ExoPlayer.Builder(ctx, renderersFactory)
                                        ...
                                        llMute.setOnClickListener {
                                            if (isMute) {
                                                if ((ctx as MainActivity).getAudioPlayerPlayingStatus() == Constant.playing) {
                                                    (ctx as MainActivity).stopAudioPlayer()
                                                }
                                                simpleExoplayer?.volume = currentVolume
                                                ivMuteUnmute.setImageDrawable(
                                                    ctx.faDrawable(
                        ...
                        GestureDetector(ctx, object : GestureDetector.SimpleOnGestureListener() {
                            ...
                            override fun onScroll(
                                ...
                                try {
                                    ...
                                    val deltaX: Float = e1?.x!! - startevent.x
                                    val deltaY: Float = e1?.y!! - startevent.y
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): RecyclerView.ViewHolder {
        ...
        if (viewType == ROW_ITYPE_1) {
            ...
            marginStart = 0.0
            marginEnd = ctx.resources.getDimensionPixelSize(R.dimen.dimen_12).toDouble()
            marginTop = commonTopMargin
            marginBottom = 0.0
            itemPaddingStart = 0.0
            itemPaddingEnd = 0.0
...
//                .toDouble() + ctx.resources.getDimensionPixelSize(R.dimen.dimen_4).toDouble()
        ...
        }else if (viewType == ROW_ITYPE_50) {
            return IType50ViewHolder(LayoutInflater.from(ctx).inflate(R.layout.row_itype_50, parent, false))
        }
        else if (viewType == ROW_ITYPE_52) {
            marginStart = 0.0
            marginEnd = ctx.resources.getDimensionPixelSize(R.dimen.dimen_20).toDouble()
            marginTop = commonTopMargin
            marginBottom = 0.0
            ...
            imageHeightByAspectRatio = itemWidth * 3 / 2
            itemHeight = imageHeightByAspectRatio + textSize + marginTop + marginBottom + itemPaddingTop + itemPaddingBottom
            return IType52ViewHolder(
                LayoutInflater.from(ctx).inflate(R.layout.row_itype_52_dynamic, parent, false)
            )
        }
        else if (viewType == ROW_ITYPE_56) {
            marginStart = 0.0
            marginEnd = ctx.resources.getDimensionPixelSize(R.dimen.dimen_32).toDouble()
            marginTop = commonTopMargin
            marginBottom = 0.0
            itemPaddingStart = 0.0
            itemPaddingEnd = 0.0
            ...
            imageHeightByAspectRatio = itemWidth * 1 / 1
            itemHeight = imageHeightByAspectRatio + textSize + marginTop + marginBottom
            return IType56ViewHolder(
                LayoutInflater.from(ctx).inflate(R.layout.row_itype_56, parent, false)
            )
        }
        else if (viewType == ROW_ITYPE_57) {
            marginStart = 0.0
            marginEnd = ctx.resources.getDimensionPixelSize(R.dimen.dimen_16).toDouble()
            marginTop = commonTopMargin
            marginBottom = 0.0
            itemPaddingStart = 0.0
            itemPaddingEnd = 0.0
            ...
            imageHeightByAspectRatio = itemWidth * 1 / 1
            itemHeight = imageHeightByAspectRatio + textSize + marginTop + marginBottom
            return IType57ViewHolder(
                LayoutInflater.from(ctx).inflate(R.layout.row_itype_57, parent, false)
            )
        }
            ...
            return IType25ViewHolder(LayoutInflater.from(ctx).inflate(R.layout.row_itype_1001, parent, false))
        }
    ...
    override fun onBindViewHolder(holder: RecyclerView.ViewHolder, row_position: Int) {
        ...
        if (position < list.size) {
            setLog("onBindViewHolderData", "" + position + " " + list[position]?.itype.toString() + " " + list[position]?.data?.title)
            ...
            } else if (list[position]?.itype == ROW_ITYPE_50) {
                (holder as IType50ViewHolder).bind(position)
            } else if (list[position]?.itype == ROW_ITYPE_51) {
                (holder as IType52ViewHolder).bind(position)
            } else if (list[position]?.itype == ROW_ITYPE_52) {
                (holder as IType52ViewHolder).bind(position)
            } else if (list[position]?.itype == ROW_ITYPE_56) {
                (holder as IType56ViewHolder).bind(position)
            } else if (list[position]?.itype == ROW_ITYPE_57) {
                (holder as IType57ViewHolder).bind(position)
