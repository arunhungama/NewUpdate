...
class UserViewModel : ViewModel() {
    ...
    var live_satus = MutableLiveData1<String>()
    private val TAG = javaClass.simpleName
     var live_date = MutableLiveData<LibraryAllRespModel?>()
...
//        mainJson.put("countryCode", " ")
    ...
    fun getUserFollowData(context: Context, userId: String, contentType: String): MutableLiveData1<Resource<FollowModel>> {
        val userRepos=UserRepos()
        return userRepos.getUserFollowData(context,userId!!,contentType)
    }
    ...
    fun getUserBookmarkedDataAll(context: Context, module:Int, typeId:String? = null): MutableLiveData1<Resource<BookmarkDataModel>> {
        val userRepos=UserRepos()
        return userRepos.getUserBookmarkedDataAll(context)
    }
    ...
    fun getLibraryAllData(context: Context,json:JSONObject): MutableLiveData1<Resource<LibraryAllRespModel>> {
    ...
    fun getLibraryAllDataNew(context: Context): MutableLiveData1<Resource<LibraryAllRespModel>> {
    ...
    fun setLibraryAllRespModelData(libraryAllRespModel: LibraryAllRespModel?) {
        live_date.value = libraryAllRespModel
    }
    fun getLibraryAllRespModelData(): MutableLiveData<LibraryAllRespModel?> {
        return live_date
    }

    fun tataplay_subscription(context: Context,json: String) {
        CoroutineScope(Dispatchers.IO).launch {

            val url =  WSConstants.url_tata_player

            val requestTime = DateUtils.getCurrentDateTime()
            DataManager.getInstance(context)?.postVolleyRequest(context, url,JSONObject(json),
                object : DataValues {
                    override fun setJsonDataResponse(response: JSONObject?) {

                        var status = response?.getString("status")
                        live_satus.value = status.toString()
                        ...
                        hashMap.put(EventConstant.NAME_EPROPERTY, "deplink_tataplay")
                        hashMap.put(
