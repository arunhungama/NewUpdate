...
import kotlinx.android.synthetic.main.fragment_search_all_tab.shimmerLayout
import kotlinx.android.synthetic.main.fragment_search_all_tab.tabs
import kotlinx.android.synthetic.main.fragment_search_all_tab.vpTransactions
...
//                            Utils.showSnakbar(requireContext(),requireView(), true, it.message!!)
            ...
            CommonUtils.showToast(requireContext(), messageModel,"SearchAllTabFragment","callSearchApi")
        }
    ...
    private fun setAllTabs(searchRespModel: SearchRespModel) {
        if (searchRespModel?.data != null && searchRespModel?.data?.body != null){
            ...
            if (!searchRespModel?.data?.body?.audio_book_episode.isNullOrEmpty()) {
                ...
                fragmentName.add(getString(R.string.search_str_25))
            }
            if (!searchRespModel?.data?.body?.audio_stories_episode.isNullOrEmpty()) {
                ...
                fragmentName.add(getString(R.string.search_str_26))
            }
            if (!searchRespModel?.data?.body?.radio.isNullOrEmpty()) {
                fragmentList.add(
                    SearchAllFragment(
                    searchRespModel,
                    radio, callSearchResultClicked)
                )
    ...
    private fun loadRecommandation(){
        ...
        if (ConnectionUtil(activity).isOnline) {
            ...
            val messageModel = MessageModel(getString(R.string.toast_message_5), getString(R.string.toast_message_5),
                MessageType.NEGATIVE, true)
            CommonUtils.showToast(requireContext(), messageModel,"SearchAllTabFragment","loadRecommandation")
        }
    }
                        ...
                        com.hungama.music.data.webservice.utils.Status.SUCCESS -> {
                            ...
                            if (!it?.data?.data?.isNullOrEmpty()!!) {
                                ...
                                it.data.data?.forEach {
                                    ...
                                    model.data?.movierights = listOf(it.movierights)
...
//            Collections.reverse(rowList)
    ...
    override fun searchItemClick(searchdata: BodyRowsItemsItem, position: Int) {
        ...
        if (searchdata?.data?.type?.toInt()==21||searchdata?.data?.type?.toInt()==110||searchdata?.data?.type?.toInt()==77777||searchdata?.data?.type?.toInt()==34) {
            if (searchdata?.data?.type?.toInt()==21 || searchdata?.data?.type?.toInt()==34){
                ...
                if (!CommonUtils.isUserHasGoldSubscription() && searchdata?.data?.misc?.movierights?.let {
                        ...
                        Log.d("sarvesh", "" + it)
                        CommonUtils.checkMovieRight(it) } == true){
                ...
                if (!CommonUtils.isUserHasGoldSubscription()){
                    if (searchdata?.data?.misc?.movierights?.let { CommonUtils.checkMovieRight(it) } == true) {
                        CommonUtils.openSubscriptionDialogPopupNew(requireContext())
                        return
                    }
                }
        ...
        }else if (searchdata?.data?.type?.toInt()==117){
            if (!CommonUtils.isUserHasGoldSubscription() && searchdata?.data?.misc?.movierights?.let {
                        it1 -> CommonUtils.checkMovieRight(it1) } == true){
                CommonUtils.openSubscriptionDialogPopupNew(requireContext())
                return
            ...
            playableItem = searchdata
            playableItemPosition = position
            CONTENT_TYPE = Constant.CONTENT_Audio_book.toInt()
            getPlayableContentUrl(searchdata?.data?.id!!)
        }else {
    ...
    fun getPlayableContentUrl(id:String){
        if (ConnectionUtil(context).isOnline) {
            ...
            val messageModel = MessageModel(getString(R.string.toast_str_35), getString(R.string.toast_message_5),
                MessageType.NEGATIVE, true)
            CommonUtils.showToast(requireContext(), messageModel,"SearchAllTabFragment","getPlayableContentUrl")
        }
    }
    ...
    private fun getPlayableMoodRadioList(id:String, type:Int){
        if (ConnectionUtil(context).isOnline) {
            ...
            val messageModel = MessageModel(getString(R.string.toast_str_35), getString(R.string.toast_message_5),
                MessageType.NEGATIVE, true)
            CommonUtils.showToast(requireContext(), messageModel,"SearchAllTabFragment","getPlayableMoodRadioList")
        }
    }
...
//            showRecentSearch(false)
    ...
    private fun callAutoSuggestionAPI(searchText: String){
        ...
        if (ConnectionUtil(activity).isOnline) {
            ...
            val messageModel = MessageModel(getString(R.string.toast_message_5), getString(R.string.toast_message_5),
                MessageType.NEGATIVE, true)
            CommonUtils.showToast(requireContext(), messageModel,"SearchAllTabFragment","callAutoSuggestionAPI")
        }
    }
