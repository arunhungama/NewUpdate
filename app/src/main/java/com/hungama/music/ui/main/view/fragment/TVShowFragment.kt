...
class TVShowFragment : BaseFragment(), OnParentItemClickListener, TracksContract.View, BaseActivity.OnDownloadVideoQueueItemChanged,
    ...
    private val broadCastReceiverTV : BroadcastReceiver = object : BroadcastReceiver() {
        override fun onReceive(contxt: Context?, intent: Intent?) {
            setLog("alhgash", "TVSHOwFragment")
            if (intent?.getIntExtra("EVENT",0) == Constant.TVSHOW_DOWNLOAD_RESULT_CODE){
                if (intent.hasExtra("index") && seasonList.get(0).data.misc.tracks.size > intent.getIntExtra("index", 0)){
                    detailTVShowAdapter?.notifyItemChanged(intent.getIntExtra("index", 0))
                }
            }
                ...
                if (detailTVShowAdapter != null) {
                    if (downloadQueue != null) {
                        ...
                        if (index != null) {
                            ...
                            intent1.putExtra(Constant.MUSIC_VIDEO_STATUS,Constant.Download)
    ...
    private fun setLocalBroadcast(){
        ...
        HungamaMusicApp.hungamaMusicApp?.applicationContext?.let {
            LocalBroadcastManager.getInstance(
                it
            ).registerReceiver(broadCastReceiverTV, IntentFilter(Constant.TVSHOW_DOWNLOAD_EVENT))
        }
    }
