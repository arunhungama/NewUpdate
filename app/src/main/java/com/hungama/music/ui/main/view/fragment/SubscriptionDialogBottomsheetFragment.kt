...
//        }
        ...
        if (Constant.DEFAULT_COUNTRY_CODE.equals("IN", true) || country_check.equals("no")) {
                ...
                if (ft != null || nonft != null) {
                    ...
                    playableContentViewModel.planInfoData.observe(this, Observer {
                        ...
                        if (CommonUtils.isUserHasEliableFreeContent() && ft?.buttonText1.equals("macro_payment_buy_ft_cta")) {
                            ...
                            tvUpgradePlan.text = String.format(
                                getString(R.string.macro_payment_buy_ft_cta),
                                it.data?.planInfo?.planName?.lowercase()
                            )

                        } else if (CommonUtils.isUserHasEliableFreeContent() && ft?.buttonText1.equals(
                                "macro_payment_buy_nonft_cta"
                            )
                        ) {

                            tvUpgradePlan.text = String.format(
                                getString(R.string.macro_payment_buy_nonft_cta),
                                it.data?.planInfo?.planCurrencySymbol,
                                if (it.data?.planInfo?.planCurrency == "INR") it.data?.planInfo?.planPrice.toString()
                                    .replace(".0", "") else it.data?.planInfo?.planPrice,
                                it.data?.planInfo?.planName?.lowercase()
                            )

                        } else if (!CommonUtils.isUserHasEliableFreeContent() && nonft?.buttonText1.equals(
                                "macro_payment_buy_ft_cta"
                            )
                        ) {
...
//            plan_type = "live_concert"
        ...
        url += "&appsflyer_id=${AppsFlyerLib.getInstance().getAppsFlyerUID(requireContext())}"
...
//            plan_type = "live_concert"
            ...
            url2 = "https://chpayments.hungama.com/plan?"
            plan_type = "subscription"
        }
        ...
        url2 += "&appsflyer_id=${AppsFlyerLib.getInstance().getAppsFlyerUID(requireContext())}"
    ...
    fun callPayApi() {
        if (ConnectionUtil(activity).isOnline) {
            ...
            CommonUtils.showToast(requireContext(), messageModel,"SubscriptionDialogBottomsheetFragment","callPayApi")
        }

    }

    override fun onDestroy() {
        if (triggerPoint == Constant.nudge_playlist_banner|| triggerPoint == Constant.nudge_album_banner || triggerPoint == Constant.nudge_player_banner  || triggerPoint == Constant.nudge_home_header_banner) {
...
//                                                    hashMap.put(EventConstant.LOAD_TIME,diff.toString())
    ...
    private fun callUpdatePaymentApi(){
        if(isAdded){
            ...
            if (ConnectionUtil(requireContext()).isOnline) {
                ...
                CommonUtils.showToast(requireContext(), messageModel,"SubscriptionDialogBottomsheetFragment","callUpdatePaymentApi")
            }
        }
    }
    ...
    fun setTextviewDataDrawer(trigger_point: String) {
        if (CommonUtils.getFirebaseConfigAdsData().enablePaymentDrawer) {
            when (trigger_point) {
                ...
                Constant.drawer_streaming_podcast -> {
                    tvTitle.text = getString(R.string.drawer_streaming_podcast_heading)
                    tvSubTitle.text = getString(R.string.drawer_streaming_podcast_subheading)
                    tvImage1.text = getString(R.string.drawer_svod_download_image1)
                    tvImage2.text = getString(R.string.drawer_svod_download_image2)
                    tvImage3.text = getString(R.string.drawer_svod_download_image3)
                    tvImage4.text = getString(R.string.drawer_svod_download_image4)

                    CommonUtils.getFirebaseConfigAdsData().drawer_streaming_podcast?.let {
                        checkValidationJsonData(it)
                    }
                }
                Constant.drawer_genric1_subscription -> {
                    tvTitle.text = getString(R.string.drawer_genric1_subscription_heading)
                    tvSubTitle.text = getString(R.string.drawer_genric1_subscription_subheading)
                    tvImage1.text = getString(R.string.drawer_genric1_subscription_image1)
                    tvImage2.text = getString(R.string.drawer_genric1_subscription_image2)
                    tvImage3.text = getString(R.string.drawer_genric1_subscription_image3)
                    tvImage4.text = getString(R.string.drawer_genric1_subscription_image4)

                    CommonUtils.getFirebaseConfigAdsData().drawer_genric1_subscription?.let {
                        checkValidationJsonData(it)
                    }
                }

                Constant.drawer_streaming_sleeptimer -> {
                tvTitle.text = getString(R.string.drawer_streaming_sleeptimer_heading)
                tvSubTitle.text = getString(R.string.drawer_streaming_sleeptimer_subheading)
                tvImage1.text = getString(R.string.drawer_streaming_sleeptimer_image1)
                tvImage2.text = getString(R.string.drawer_streaming_sleeptimer_image2)
                tvImage3.text = getString(R.string.drawer_streaming_sleeptimer_image3)
                tvImage4.text = getString(R.string.drawer_streaming_sleeptimer_image4)

                CommonUtils.getFirebaseConfigAdsData().drawer_streaming_sleeptimer?.let {
                    checkValidationJsonData(it)
                }
            }
                Constant.drawer_streaming_mymix -> {
                tvTitle.text = getString(R.string.drawer_streaming_mymix_heading)
                tvSubTitle.text = getString(R.string.drawer_streaming_mymix_subheading)
                tvImage1.text = getString(R.string.drawer_streaming_mymix_image1)
                tvImage2.text = getString(R.string.drawer_streaming_mymix_image2)
                tvImage3.text = getString(R.string.drawer_streaming_mymix_image3)
                tvImage4.text = getString(R.string.drawer_streaming_mymix_image4)

                CommonUtils.getFirebaseConfigAdsData().drawer_streaming_mymix?.let {
                    checkValidationJsonData(it)
                }
            }
            }
    ...
    private fun setTextviewDataNudge(trigger_point: String) {
        if (CommonUtils.getFirebaseConfigAdsData().enablePaymentNudge) {
            when (trigger_point) {
                ...
                Constant.nudge_album_banner -> {
                    setDefaultView()
                    if (CommonUtils.isUserHasEliableFreeContent()) ft =
                        CommonUtils.getFirebaseConfigAdsData().nudgeAlbumBanner.ft
                    else nonft =
                        CommonUtils.getFirebaseConfigAdsData().nudgeAlbumBanner.nonft

                }
                Constant.nudge_home_header_banner -> {
                setDefaultView()
                if (CommonUtils.isUserHasEliableFreeContent()) ft =
                    CommonUtils.getFirebaseConfigAdsData().nudgeHomeHeaderBanner.ft
                else nonft =
                    CommonUtils.getFirebaseConfigAdsData().nudgeHomeHeaderBanner.nonft
    ...
    fun checkValidationJsonData(data: AdsConfigModel.DrawerDownloadAll) {
        ...
        if (CommonUtils.isUserHasEliableFreeContent()) {
            if (data?.ft != null) {
                ft = data?.ft
                if(bannerFlag.isNotEmpty())
                enable_drawer = data.enable_drawer
    ...
    fun RedirectToWebpage() {
        ...
        if(enable_drawer.equals("false")){
            bannerFlag="see_all"
        }

        if(bannerFlag.equals("see_all")){
        planId =""
        }
    ...
    fun EventviewFunction(trigger_point: String) {
        if(enable_drawer.equals("false")){
            ...
            if (trigger_point == Constant.nudge_playlist_banner || trigger_point == Constant.nudge_album_banner || trigger_point == Constant.nudge_player_banner || trigger_point == Constant.nudge_home_header_banner) {
