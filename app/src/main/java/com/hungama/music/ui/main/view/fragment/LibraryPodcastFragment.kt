...
class LibraryPodcastFragment : BaseFragment(), CreatePlaylistDialog.createPlayListListener,
    ...
    override fun initializeComponent(view: View) {
        baseMainScope.launch {
            ...
            btnExplore.setOnClickListener {
                try {
                    if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.M) {
                        CommonUtils.hapticVibration(requireContext(), btnExplore, HapticFeedbackConstants.CONTEXT_CLICK)
                    }
                }catch (e:Exception){
            ...
            musicplayList.add(LibraryMusicModel("" + FOLLOW_MORE_PODCAST_ID, getString(R.string.podcast_str_13), "", "", ""))

            val downloadingPodcastTotal = AppDatabase?.getInstance()?.downloadQueue()?.getDownloadQueueItemsByContentType(ContentTypes.PODCAST.value)
            ...
            if (totalPadcast != 0){
                musicplayList.add(LibraryMusicModel(""+ DOWNLOADING_PROGRESS, getString(R.string.library_all_str_4), downloadingSubtitle, "", ""))
            }
            ...
            if (totalDownloadedPodcast != 0){
                musicplayList.add(LibraryMusicModel(""+ DOWNLOADED_PODCAST, getString(R.string.download_str_10), downloadedPodcastSubtitle, "", ""))
            }
