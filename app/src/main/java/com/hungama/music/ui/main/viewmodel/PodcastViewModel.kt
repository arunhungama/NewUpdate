...
class PodcastViewModel : ViewModel() {
    ...
    fun getAudioBookDetailList(context: Context, id:String): MutableLiveData<Resource<PlaylistDynamicModel>>? {
        podcastRepos=PodcastRepos()
        return podcastRepos?.getAdioBookDetailList(context,id)
    }

    fun getAudioBookStoryEpisodeList(context: Context, id:String, page:Int, isAudioBook:Boolean): MutableLiveData<Resource<PlaylistDynamicModel>>? {
        podcastRepos=PodcastRepos()
        return if (isAudioBook) podcastRepos?.getAudioBookEpisodeList(context,id, page) else
            podcastRepos?.getAudioStoryEpisodeList(context,id, page)
    }
