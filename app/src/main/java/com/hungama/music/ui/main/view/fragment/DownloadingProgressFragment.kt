    ...
    private fun getDownloadQueueSongList() {
        try {
            if(isAdded){
                    ...
                    downloadQueuetList = AppDatabase?.getInstance()?.downloadQueue()?.getDownloadQueueItemsByAudioContentType(ContentTypes.AUDIO.value, ContentTypes.PODCAST.value) as ArrayList<DownloadQueue>?
                    val audioBookStory = AppDatabase?.getInstance()?.downloadQueue()?.getDownloadQueueItemsByAudioContentType(ContentTypes.AUDIO_BOOK.value, ContentTypes.AUDIO_STORY.value) as ArrayList<DownloadQueue>?
                if (audioBookStory != null){
                    downloadQueuetList?.addAll(audioBookStory)
                }
