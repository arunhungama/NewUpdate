...
import com.hungama.music.nativetemplates.TemplateView
...
class DetailAudioBookAdapter(
    ...
    //    val ivUserImage: ImageView = itemView.findViewById(R.id.ivUserImage)
        ...
        val rlChildMain: RelativeLayout = itemView.findViewById(R.id.rlChildMain)
                ...
                if (currentPlayingContentId?.toString()?.equals(dataItemModel?.id)!!) {
                    ...
                    ivPlay?.setOnClickListener {
                        ...
                        if (!CommonUtils.isUserHasGoldSubscription()){
                                ...
                                if (CommonUtils.checkMovieRight(dataItemModel.misc.movierights)){
...
//                }
            ...
            if (!CommonUtils.isUserHasGoldSubscription())
                if (dataItemModel.type == 114)
                CommonUtils.checkAudioBookCheck(ctx, dataItemModel.misc.movierights.toString(), ivfreeStrips, rlChildMain)
            else
                    CommonUtils.checkAudioStoriesCheck(ctx, dataItemModel.misc.movierights.toString(), ivfreeStrips, rlChildMain)
            else{
                ivfreeStrips?.hide()
            ...
            setLog("algha","$formattedDate")
            var duration = dataItemModel.duration.toInt()
            var hours = duration / 3600
            val minute = (duration % 3600) / 60
            val seconds = duration % 60

            tvSubTitle.text = "$minute min $seconds sec"
            ...
            ivDownload.setOnClickListener {
                if (onChildItemClick != null) {
                    if (CommonUtils.isUserHasGoldSubscription())
                        downloadIconStates(1, ivDownload)
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): RecyclerView.ViewHolder {
        ...
        if (viewType == Constant.podcastNativeAds){
            ...
            return IType1000ViewHolder(LayoutInflater.from(ctx).inflate(R.layout.row_audio_book_detail, parent, false))
        }
    }
    ...
    private fun initNativeAds(
        ...
        adLoader = AdLoader.Builder(ctx, list.adUnitId).forNativeAd(object : NativeAd.OnNativeAdLoadedListener{
