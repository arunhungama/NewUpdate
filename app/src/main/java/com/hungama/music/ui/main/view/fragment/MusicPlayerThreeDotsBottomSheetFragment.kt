...
class MusicPlayerThreeDotsBottomSheetFragment() : SuperBottomSheetFragment(), MusicPlayBackSettingStreamQuality.OnItemClick,
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        if (currentTrack?.playerType.equals(Constant.CONTENT_Audio_book)
                || currentTrack?.playerType.equals(Constant.CONTENT_Audio_Story)){
            rl_music_player_menu_go_to_audio_book.visibility = View.VISIBLE
            rl_music_player_menu_playback_speed.visibility = View.VISIBLE
            rl_music_player_menu_playlist?.visibility = View.GONE
            rl_music_player_menu_song_details?.visibility = View.GONE
            txt_music_player_menu_audio_quality.text = getString(R.string.audio_books_str_threedots_streamingquality)
            iv_music_player_menu_audio_quality.setImageDrawable(
                context?.resources?.getDimensionPixelSize(R.dimen.font_17)?.toFloat()?.let {
                    context?.faDrawable(
                        R.string.icon_stream_quality,
                        R.color.colorWhite,
                        it
                    )
                }
            )
            txt_music_player_menu_go_to_audio_book.text =  if (currentTrack?.playerType.equals(Constant.CONTENT_Audio_book)) getString(R.string.audio_books_str_threedots) else getString(R.string.audio_stories_str_threedots)
        }
        ...
        rl_music_player_menu_view_album.setOnClickListener {
            if (activity is BaseActivity) {
                ...
                if (!pidList.isNullOrEmpty()){
                    ...
                    CommonUtils.showToast(requireContext(), messageModel,"MusicPlayerThreeDotsBottomSheetFragment","onViewCreated")
                }
            }
        ...
        rl_music_player_menu_go_to_audio_book.setOnClickListener {
            closeDialog()
            if (BaseActivity.isDisplayDiscover){
                (activity as BaseActivity).toggleSheetBehavior()
            }
            ...
            if (activity is BaseActivity) {
                ...
                if (currentTrack?.playerType.equals(Constant.CONTENT_Audio_book) || currentTrack?.playerType.equals(Constant.CONTENT_Audio_Story)){
                    val bundle = Bundle()
                    bundle.putString("id",""+currentTrack?.parentId)
                    bundle.putString("image", ""+currentTrack?.pImage)
                    bundle.putString("playerType", ""+currentTrack?.playerType)
                    val podcastDetailFragment = AudioBookDetailsFragment()
                    podcastDetailFragment.arguments = bundle
                    if (activity != null && activity is MainActivity){
                        (activity as MainActivity).unSelectAllTab()
                    }
                    ...
                    val songDetailFragment = AudioBookDetailsFragment()
                    songDetailFragment.arguments = bundle
...
/*
    ...
    private fun setUpMoodRadioPopupTempoViewModel() {
        if (ConnectionUtil(requireContext()).isOnline) {
            ...
            val messageModel = MessageModel(getString(R.string.toast_str_35), getString(R.string.toast_message_5),
                MessageType.NEGATIVE, true)
            CommonUtils.showToast(requireContext(), messageModel,"MusicPlayerThreeDotsBottomSheetFragment","setUpMoodRadioPopupTempoViewModel")
        }
    }
    ...
    private fun setUpMoodRadioPopupLanguageViewModel() {
        ...
        if (ConnectionUtil(requireContext()).isOnline) {
            ...
            val messageModel = MessageModel(getString(R.string.toast_str_35), getString(R.string.toast_message_5),
                MessageType.NEGATIVE, true)
            CommonUtils.showToast(requireContext(), messageModel,"MusicPlayerThreeDotsBottomSheetFragment","setUpMoodRadioPopupLanguageViewModel")
        }
...
//            tvTempoTitle.text = moodRadioPopupTempoModel.data?.body?.rows?.get(0)?.items?.get(position)?.title.toString()
    ...
    private fun setUpMoodRadioContentList() {
        ...
        if (ConnectionUtil(requireContext()).isOnline) {
            ...
            val messageModel = MessageModel(getString(R.string.toast_str_35), getString(R.string.toast_message_5),
                MessageType.NEGATIVE, true)
            CommonUtils.showToast(requireContext(), messageModel,"MusicPlayerThreeDotsBottomSheetFragment","setUpMoodRadioContentList")
        }
    }
    ...
    fun getPlayableContentUrl(id:String){
        if (ConnectionUtil(context).isOnline) {
            ...
            val messageModel = MessageModel(getString(R.string.toast_str_35), getString(R.string.toast_message_5),
                MessageType.NEGATIVE, true)
            CommonUtils.showToast(requireContext(), messageModel,"MusicPlayerThreeDotsBottomSheetFragment","getPlayableContentUrl")
        }
    }
