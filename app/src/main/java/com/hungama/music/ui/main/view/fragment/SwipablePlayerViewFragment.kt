...
import kotlinx.android.synthetic.main.swipable_player_native_ads_view.clProgressView
import kotlinx.android.synthetic.main.swipable_player_native_ads_view.pb
import kotlinx.android.synthetic.main.swipable_player_native_ads_view.tvProgress
...
//        BucketChildAdapter.simpleExoplayer?.pause()
        headBarBlur?.hide()
        ...
        mainScope.launch {
            if (isAdded) {
                ...
                if(track?.heading.equals("")) {
                    text_view_parent_tag?.text = track?.pName
                }
                else{
                    text_view_parent_tag?.text = track?.heading
                }
                setLog("ahgasg", "${text_view_parent_tag?.text} visible ${text_view_parent_tag?.isVisible}")
...
//                        setLog("ahlgla", CommonUtils.getFirebaseConfigAdsData().nudgeAlbumBanner.ft!!.toString())
                            ft = CommonUtils.getFirebaseConfigAdsData().nudgePlayerBanner.ft
                        ...
                        nonft = CommonUtils.getFirebaseConfigAdsData().nudgePlayerBanner.nonft
...
//                            }
                ...
                if (track?.playerType == "117"){
                    image_view_music_icon?.setImageDrawable(
                        requireContext().faDrawable(
                            R.string.icon_go_to_audio_story,
                            R.color.colorWhite,
                            resources.getDimensionPixelSize(R.dimen.font_18).toFloat()
                        )
                    )
                    text_view_parent_tag?.setTextOrHide(value = getString(R.string.search_str_26))
                    roundViewOne.hide()
                    ivFavoriteAnim.hide()
                    image_view_option_like_title.hide()
                }
                else if(track?.playerType == "114"){
                    image_view_music_icon?.setImageDrawable(
                        requireContext().faDrawable(
                            R.string.icon_go_to_audio_book,
                            R.color.colorWhite,
                            resources.getDimensionPixelSize(R.dimen.font_18).toFloat()
                        )
                    )
                    text_view_parent_tag?.setTextOrHide(value = getString(R.string.search_str_25))
                    roundViewOne.hide()
                    ivFavoriteAnim.hide()
                    image_view_option_like_title.hide()
                }
    ...
    override fun onPause() {
        ...
        setLog(
            ...
            "SwipablePlayerViewFragment-onPause()-songPosition=${songPosition} dataSize ${BaseActivity.songDataList.size} currentViewPagerIndex:${SwipablePlayerFragment.currentViewPagerIndex} nowPlayingCurrentIndex:${BaseActivity.nowPlayingCurrentIndex()}"
        )
        super.onPause()
    }
                ...
                if (songPosition > -1) {
                    if (BaseActivity.nowPlayingCurrentIndex() == songPosition) {
                        ...
                        swipableAdsView?.hide()
                            ...
                            intent.putExtra(
                                Constant.playerArtworkChange,
                                BaseActivity.songDataList.get(songPosition)?.image.toString()
                            )
...
/*
                            if (!CommonUtils.checkGoldMusicPodcast()){
                                    val intent = Intent(Constant.AUDIO_PLAYER_UI_EVENT)
                                    intent.putExtra(Constant.playerArtworkChange, BaseActivity.songDataList.get(songPosition)?.image.toString())
                                    intent.putExtra("EVENT", Constant.AUDIO_PLAYER_UI_RESULT_CODE)
                                    LocalBroadcastManager.getInstance(requireContext()).sendBroadcast(intent)
                                var list_movierights: ArrayList<String> = ArrayList<String>()
                                list_movierights.add(BaseActivity.songDataList.get(BaseActivity.nowPlayingCurrentIndex()).movierights)
                                if(CommonUtils.checkMovieRight(list_movierights))
                                return@launch
                            }*/
        ...
        if (BaseActivity.isChromeCasted){
            (activity as MainActivity).stopAudioPlayer()
        }
        super.onResume()
    }
    ...
    override fun onClick(v: View?) {
        if (v == ivPlayPause) {
            ...
            if (activity != null && activity is MainActivity){
                ...
                when {
                    ...
                    (activity as MainActivity).getAudioPlayerPlayingStatus() == Constant.pause -> {
                            ...
                            if(CommonUtils.checkGoldMusic(BaseActivity.songDataList.get(songPosition).movierights,
                               BaseActivity.songDataList.get(songPosition).contentType)){
                                CommonUtils.openSubscriptionDialogPopupNew(requireContext());  return
                            }
                        (activity as MainActivity).playPlayer()
                        if (currentPlayerType == Constant.CONTENT_RADIO) {
                            ivPlayPause?.setImageDrawable(
            ...
            }else if(track!=null && !TextUtils.isEmpty(track?.shareUrl)){
                setLog("TAG", "rl_music_player_menu_share share url:${track?.shareUrl}")
                var shareurl = getString(R.string.music_player_str_18) + " " + track?.shareUrl
                shareurl += "play/"
                Utils.shareItem(requireActivity(), shareurl)
            }else {
                try {
            ...
            mAdManagerInterstitialAd?.fullScreenContentCallback = object : FullScreenContentCallback() {
                override fun onAdDismissedFullScreenContent() {
                    ...
                    if (activity != null) {
                        if (!CommonUtils.isUserHasGoldSubscription()){
                            var list_movierights: ArrayList<String> = ArrayList<String>()
                            list_movierights.add(track?.movierights.toString())
            ...
            if (track?.pType == DetailPages.PLAYLIST_DETAIL_PAGE.value || track?.pType == DetailPages.PLAYLIST_DETAIL_ADAPTER.value)
            {
                if (activity is BaseActivity) {
                    ...
                    if (!TextUtils.isEmpty(track.parentId)){
                        ...
                        CommonUtils.showToast(requireContext(), messageModel,"SwipablePlayerViewFragment","redirectToDetailPage")
                    }
                }
            }
            else if (track?.pType == DetailPages.CHART_DETAIL_PAGE.value || track?.pType == DetailPages.CHART_DETAIL_ADAPTER.value)
            {
                if (activity is BaseActivity) {
                    ...
                    if (!TextUtils.isEmpty(track.parentId)){
                        ...
                        val messageModel = MessageModel(getString(R.string.no_playlist_found), MessageType.NEUTRAL, true)
                        CommonUtils.showToast(requireContext(), messageModel,"SwipablePlayerViewFragment","callPayApi")
                    }
                }
            }
            else if (track?.pType == DetailPages.ALBUM_DETAIL_PAGE.value || track?.pType == DetailPages.ALBUM_DETAIL_ADAPTER.value || track?.pType == DetailPages.RECOMMENDED_SONG_LIST_PAGE.value)
            {
                if (activity is BaseActivity) {
                    val playableContentModel=HungamaMusicApp.getInstance().getEventData(""+track?.id.toString())
                    val bundle = Bundle()
