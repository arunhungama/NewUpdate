...
class SearchAllFragment(
    ...
    override fun initializeComponent(view: View) {
        ...
        else if (tabId == SearchAllTabFragment.audioBooks) {
            if (!searchRespModel?.data?.body?.audio_book_episode.isNullOrEmpty()) {
                searchRespModel.data?.body?.audio_book_episode?.let { rowList.addAll(it) }
                CoroutineScope(Dispatchers.IO).launch {
                    calSearchResultFilterEvent(SearchAllTabFragment.lastSearchText.trim(),""+searchRespModel?.data?.body?.audio_book_episode?.size, SearchAllTabFragment.lastSearchText.trim(),"audiobooks")
                }
            }
            pageName = "audio_books"
        }
        else if (tabId == SearchAllTabFragment.audioStory) {
            if (!searchRespModel?.data?.body?.audio_stories_episode.isNullOrEmpty()) {
                searchRespModel.data?.body?.audio_stories_episode?.let { rowList.addAll(it) }
                CoroutineScope(Dispatchers.IO).launch {
                    SearchAllTabFragment.lastSearchText?.trim()?.let {
                        calSearchResultFilterEvent(SearchAllTabFragment.lastSearchText.trim(),""+searchRespModel?.data?.body?.audio_stories_episode?.size,
                            it,"audiostory")
                    }
                }
            }
            pageName = "audio_story"
        }
...
//            searchAdapter.setSearchlist(rowList!!)
    ...
    override fun searchItemClick(searchdata: BodyRowsItemsItem, position: Int) {
        ...
        if(!searchdata?.data?.type?.isEmpty()!!){
            setLog("lahglahs", "${searchdata?.data?.type}")
            ...
            if (searchdata?.data?.type?.toInt()==21||searchdata?.data?.type?.toInt()==110||searchdata?.data?.type?.toInt()==77777||searchdata?.data?.type?.toInt()==34) {
                if (searchdata?.data?.type?.toInt()==21 || searchdata?.data?.type?.toInt()==34){
                    if (!CommonUtils.isUserHasGoldSubscription() && searchdata?.data?.misc?.movierights?.let {
                    ...
                    if (!CommonUtils.isUserHasGoldSubscription()){
                         if(searchdata?.data?.misc?.movierights?.toString()?.contains("SMOD") == true) {
                             CommonUtils.openSubscriptionDialogPopupNew(requireContext())
                             return
                         }
                    }
    ...
    fun getPlayableContentUrl(id:String){
        if (ConnectionUtil(context).isOnline) {
            ...
            CommonUtils.showToast(requireContext(), messageModel,"SearchAllFragment","getPlayableContentUrl")
        }
    }
    ...
    private fun getPlayableMoodRadioList(id:String, type:Int){
        if (ConnectionUtil(context).isOnline) {
            ...
            val messageModel = MessageModel(getString(R.string.toast_str_35), getString(R.string.toast_message_5),
                MessageType.NEGATIVE, true)
            CommonUtils.showToast(requireContext(), messageModel,"SearchAllFragment","getPlayableMoodRadioList")
        }
    }
    ...
    private fun callAddRecentSearh(searchdata: BodyRowsItemsItem) {
        ...
        if (ConnectionUtil(requireActivity()).isOnline) {
            ...
            addRecentSearchJson.put("movierights",searchdata?.data?.misc?.movierights.toString())
...
/*        if (!TextUtils.isEmpty(playableItem.data?.subTitle)){
    ...
    fun onItemDetailPageRedirection(searchdata: BodyRowsItemsItem, position: Int) {
        ...
        else if (type.equals("114", true) || type.equals("117", true)) {
...
//                            Utils.showSnakbar(requireContext(),requireView(), true, it.message!!)
            ...
            val messageModel = MessageModel(getString(R.string.toast_message_5), getString(R.string.toast_message_5),
                MessageType.NEGATIVE, true)
            CommonUtils.showToast(requireContext(), messageModel,"SearchAllFragment","callAllSearchPageWiseApi")
        }
