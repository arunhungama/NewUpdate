...
class VideoPlayBackSetting : BaseFragment(), VideoPlayBackSettingStreamQuality.OnItemClick {
    ...
    private fun fillUI(userSettingRespModel: UserSettingRespModel) {
        if (userSettingRespModel?.data != null && userSettingRespModel?.data?.data?.get(0)?.preference?.get(
            ...
            SharedPrefHelper.getInstance().setVideoAutoPlay(svAutoPlay.isChecked)
            if (!TextUtils.isEmpty(userSettingRespModel?.data?.data?.get(0)?.preference?.get(0)?.streaming_quality)){
                ...
                setLog("aghasfh", "$quality ${quality.id}")
            }
    ...
    private fun getSetting() {
        if (ConnectionUtil(requireContext()).isOnline) {
            ...
            CommonUtils.showToast(requireContext(), messageModel,"VideoPlayBackSetting","getSetting")
        }
    }
