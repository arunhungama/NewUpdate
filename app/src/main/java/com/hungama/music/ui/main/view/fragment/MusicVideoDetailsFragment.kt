...
import kotlinx.android.synthetic.main.fragment_video_details.progress
import kotlinx.android.synthetic.main.video_player_custum_control.*
import kotlinx.android.synthetic.main.video_player_view.loading_exoplayer
import kotlinx.android.synthetic.main.video_player_view.mainFullScreeen
...
class MusicVideoDetailsFragment : BaseFragment(), BucketParentAdapter.OnMoreItemClick,
    ...
    override fun initializeComponent(view: View) {
        ...
        intent1.putExtra(Constant.MUSIC_VIDEO_STATUS,Constant.STOP)
        HungamaMusicApp.hungamaMusicApp?.applicationContext?.let {
            LocalBroadcastManager.getInstance(it).sendBroadcast(intent1)
        }

        videoAdId = CommonUtils.getFirebaseConfigAdsData().video_prompt.video_id
        ...
        requireActivity().registerReceiver(broadcastReceiver, IntentFilter(ACTION_PIP_CONTROL),Context.RECEIVER_NOT_EXPORTED)
        else
        ...
        if(requireArguments()!=null &&requireArguments().containsKey(Constant.EXTRA_LIST)){
            musicvideoList= (requireArguments().getSerializable(Constant.EXTRA_LIST) as ArrayList<DownloadedAudio>?)!!
        ...
        if(requireArguments()!=null&&requireArguments().containsKey(Constant.CHILD_POSITION)){
            ...
            }else selectedContentId = CommonUtils.getFirebaseConfigAdsData().video_prompt.video_id
        }
        ...
        if(!CommonUtils.isUserHasGoldSubscription()) {
            ...
            if (CommonUtils.isUserHasEliableFreeContent()) {
                ...
                ft = CommonUtils.getFirebaseConfigAdsData().nudge_music_video_banner.ft             ///////////////nudge_music_video_banner
                ...
                nonft = CommonUtils.getFirebaseConfigAdsData().nudge_music_video_banner.nonft   ///////////////nudge_music_video_banner
        ...
        iv_banner.setOnClickListener {
            Constant.screen_name ="Music Video Screen"
            CommonUtils.openSubscriptionDialogPopup(requireContext(), PlanNames.SVOD.name, "", true, null, "", null,Constant.nudge_album_banner,"banner")
        }
           ...
           /* mediaSession = MediaSessionCompat(requireContext(), requireContext().packageName)
            ...
            if (currentPlayer != null && !videoList.isNullOrEmpty()) {
                ...
                if(mPlayer!=null&&videoList!=null&&videoList.get(mPlayer?.currentWindowIndex!!)!=null) {
                    tvHeading.text = videoList.get(mPlayer?.currentWindowIndex!!).data?.head?.headData?.title
                    ...
                    CoroutineScope(Dispatchers.Main).launch {
                        ...
                        val playerIndex = mPlayer?.currentWindowIndex

                        if(videoList!=null && playerIndex?.let { videoList.get(it).data } !=null
                            && videoList.get(playerIndex).data?.head!=null
                            &&videoList.get(playerIndex).data?.head?.headData !=null
                            ){
                        if(videoList.get(playerIndex).data?.head?.headData?.title !=null)
                        hashMapPageView[EventConstant.CONTENT_NAME_EPROPERTY] =
                            videoList.get(playerIndex).data.head.headData.title
                        if(videoList.get(playerIndex).data?.head?.headData?.type !=null)
                        hashMapPageView[EventConstant.CONTENT_TYPE_EPROPERTY] =
                            "" + Utils.getContentTypeNameForStream("" + videoList.get(playerIndex).data?.head?.headData?.type)
                        hashMapPageView[EventConstant.CONTENT_TYPE_ID_EPROPERTY] =
                            "" + videoList.get(playerIndex).data?.head?.headData?.id
...
//                        EventManager.getInstance().sendEvent(PageViewEvent(hashMapPageView))
                    ...
                    if(videoList.get(mPlayer?.currentMediaItemIndex!!)!=null)
                    callStreamEventAnalyticsStart(videoList.get(mPlayer?.currentMediaItemIndex!!),false)
                }
                    setUpChormeCast()
    ...
    private fun startPlayerService(item: ArrayList<PlayableContentModel>): Boolean {
        ...
        if (!item.isNullOrEmpty() && item.size > playableItemPosition) {
            val track =  Track()
            val id = item.get(playableItemPosition).data.head.headData.id
            if (!TextUtils.isEmpty(id))
            track.id = id.toLong()
            else
                setLog("trackData","trackid is empty")
...
//                startService(intent)
            ...
            if (serviceIntent != null){
                ...
                requireContext().bindService(serviceIntent!!, mConnection, AppCompatActivity.BIND_AUTO_CREATE)
                initializePlayer()
            }
...
//                    }
    ...
    private fun saveSetting(videoQuality: VideoQuality) {
        if (ConnectionUtil(requireContext()).isOnline) {
            ...
            CommonUtils.showToast(requireContext(), messageModel,"VideoPlayBackSetting","saveSetting")
        }
    }
...
//        )
    ...
    private fun getUserBookmarkedData() {
        ...
        if (ConnectionUtil(requireContext()).isOnline(isShowNoInternetPopUp)) {
            ...
            CommonUtils.showToast(requireContext(), messageModel,"MusicVideoDetailsFragment","getUserBookmarkedData")
        }
    }
    ...
    private fun setupVideoDetailViewModel() {
        ...
        if (ConnectionUtil(context).isOnline(isShowNoInternetPopUp)) {
            ...
            CommonUtils.showToast(requireContext(), messageModel,Constant.screen_name,"getVideoDetail","","")
        }
    }
...
//                videoTitle.text = videoDetailModel.data.head.data.title
    ...
    var data = PlayableContentModel()


    private fun setUpVideoListViewModel(selectedContentId: String, isForVideoAdd:Boolean = false) {
        ...
        if (ConnectionUtil(requireContext()).isOnline(isShowNoInternetPopUp)) {
            videoListViewModel?.getVideoList(requireContext(), selectedContentId, 5)?.observe(this,
                Observer {
                    when (it.status) {
                        com.hungama.music.data.webservice.utils.Status.SUCCESS -> {
                            setProgressBarVisible(false)
                            setLog(TAG, "setUpVideoListViewModel getVideoList selectedContentId ${selectedContentId}")
                            if (it?.data != null) {
                                if (!CommonUtils.isUserHasGoldSubscription()){
                                    if (isForVideoAdd)
                                        videoAddUrl = it?.data.data.head.headData.misc.url
                                    else
                                        data = it.data

                                    if (videoAddUrl.isEmpty()){
                                        if (videoAdId.isEmpty())
                                            videoAdId = CommonUtils.getFirebaseConfigAdsData().video_prompt.video_id
                                        if (videoAdId.isNotEmpty())
                                        setUpVideoListViewModel(videoAdId, true)
                                        setLog("VIdeoAddId", " $videoAdId")
                                    }
                                    else{
                                        data.data.head.headData.misc.url = videoAddUrl
            ...
            CommonUtils.showToast(requireContext(), messageModel,"MusicVideoDetailsFragment","setUpVideoListViewModel")
        }
    }
...
//        initializePlayer()
    ...
    override fun onDestroy() {
        ...
        try {
            val intent1 = Intent(Constant.TVSHOW)
            intent1.putExtra(Constant.MUSIC_VIDEO_STATUS,Constant.STOP_PLAY)
            HungamaMusicApp.hungamaMusicApp?.applicationContext?.let {
                LocalBroadcastManager.getInstance(it).sendBroadcast(intent1)
            }
    ...
    private fun setAddOrRemoveFavourite(videoDetailRespModel: PlaylistDynamicModel) {
        ...
        if (ConnectionUtil(context).isOnline(isShowNoInternetPopUp)) {
            ...
            if (isAddWatchlist) {
                ...
                CommonUtils.showToast(requireContext(), messageModel,"MusicVideoDetailsFragment","setAddOrRemoveFavourite")
                //}
...
e.printStackTrace()
    ...
    private fun registerBroadcastReceiver() {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.TIRAMISU)
        requireActivity().registerReceiver(screenOnOffReceiver, theFilter,Context.RECEIVER_NOT_EXPORTED)
        else
    ...
    private fun showMiniVideoPlayer(intent: Intent) {
        ...
        if (intent.hasExtra(Constant.VIDEO_START_POSITION)) {
