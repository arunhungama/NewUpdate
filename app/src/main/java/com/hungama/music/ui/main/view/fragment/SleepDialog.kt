...
class SleepDialog(val onSleepTimeChangeItemClicked: BaseActivity, val currentTrack: Track?): BottomSheetDialogFragment(),
    ...
    override fun onSleepTimeChangeItemClick(data: SleepDialogModel) {
        if (!CommonUtils.isUserHasGoldSubscription()){
            ...
            CommonUtils.openSubscriptionDialogPopup(requireContext(), PlanNames.SVOD.name, "", true, null, "", null,
        ...
        if (onSleepTimeChangeItemClick != null){
            ...
            CommonUtils.showToast(requireContext(), messageModel,"SleepDialog","onSleepTimeChangeItemClick")
