...
import kotlinx.android.synthetic.main.row_redeem_coins.*
    ...
    private fun getUserPlaylistData() {
        baseMainScope.launch {
            ...
            if (ConnectionUtil(requireContext()).isOnline) {
                ...
                CommonUtils.showToast(requireContext(), messageModel,"PlaylistFragment","getUserPlaylistData")
    ...
    private fun addFavPlayList() {
        ...
        if (favPlayList != null && favPlayList?.size!! > 0) {
            favPlayList?.forEach {
                ...
                var isExist = false
                for(data in userPlaylist){
                 if (playlistData.data.id == data.data.id)
                 {
                     isExist = true
                     break
                 }
