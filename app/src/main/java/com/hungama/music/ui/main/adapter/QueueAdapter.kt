...
import com.hungama.myplay.activity.databinding.RowNowPlayingBinding
import com.hungama.myplay.activity.databinding.RowQueueBinding
import com.hungama.myplay.activity.databinding.RowQueueHistoryBinding
    ...
    override fun onBindViewHolder(holder: RecyclerView.ViewHolder, position: Int) {
        ...
        if(getItemViewType(position)==ROW_TYPE_1 && holder is HistoryItemViewHolder){
            if (arrayList?.get(holder.adapterPosition)?.contentType == ContentTypes.Audio_Ad.value){
                ...
                if (arrayList?.get(holder.adapterPosition)?.image != null) {
                    ImageLoader.loadImage(
                        context,
                        (holder as HistoryItemViewHolder).binding.ivSong,
            ...
            if (arrayList?.get(holder.adapterPosition)?.image != null) {
                ImageLoader.loadImage(
                    context,
                    (holder as NowPlayingViewHolder).binding.ivNowPlay,
                    arrayList?.get(holder.adapterPosition)?.image!!,
                    R.drawable.bg_gradient_placeholder
                )
            }
            ...
            if (arrayList?.get(holder.adapterPosition)?.contentType == ContentTypes.Audio_Ad.value){
                ...
                if (arrayList?.get(holder.adapterPosition)?.image != null) {
                    ImageLoader.loadImage(
                        context,
                        (holder as ItemViewHolder).binding.ivSong,
                        arrayList?.get(holder.adapterPosition)?.image!!,
                        R.drawable.bg_gradient_placeholder
                    )
                }
