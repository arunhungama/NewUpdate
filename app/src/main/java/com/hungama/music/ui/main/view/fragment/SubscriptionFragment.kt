...
class SubscriptionFragment : BaseFragment(), OnUserSubscriptionUpdate,
    ...
    fun getUserSubscriptionStatus(){
        if (ConnectionUtil(requireContext()).isOnline) {
            ...
            CommonUtils.showToast(requireContext(), messageModel,"SubscriptionFragment","getUserSubscriptionStatus")
        }
    }
    ...
    private fun fillUI(userSubscriptionModel: UserSubscriptionModel) {
        if (userSubscriptionModel != null && userSubscriptionModel?.data != null
            ...
            if ((userSubscriptionModel.data?.subscription?.totalDays ?: 0) > 0){
                progressBarDays?.max = userSubscriptionModel?.data?.subscription?.totalDays?: 0
                progressBarDays?.progress = userSubscriptionModel?.data?.subscription?.daysRemaining?: 0
            }else{
                        ...
                        when(it.status){
                            Status.SUCCESS->{
                                if (it?.data != null && it?.data?.success!!){
                                    ...
                                    CommonUtils.showToast(requireContext(), messageModel,"SubscriptionFragment","callUnsubscribeApi")
                                    llCancelSubscription?.visibility = View.GONE
                                    //setUpViewModel()
                                }
                            }
