...
class CommonThreeDotsMenuFragment : SuperBottomSheetFragment() {
    companion object{
        ...
        var track: PlaylistModel.Data.Body.Row.Data.Misc.Track? =  null
        var forAdapter :Int = 0
        ...
        fun newInstance(threeDotMenuItemsDataModelList: ArrayList<CommonThreeDotsMenuItemModel>, onThreeDotMenuItemClick: OnThreeDotMenuItemClick, track: PlaylistModel.Data.Body.Row.Data.Misc.Track?, forAdapter:Int): CommonThreeDotsMenuFragment{
            val fragment = CommonThreeDotsMenuFragment()
            this.threeDotMenuItemsDataModelList = threeDotMenuItemsDataModelList
            this.onThreeDotMenuItemClick = onThreeDotMenuItemClick
            this.track = track
                        ...
                        object : CommonThreeDotsMunuItemAdapter.OnMenuItemClick {
                            override fun onMenuItemClick(position: Int, commonThreeDotsMenuItemModel: CommonThreeDotsMenuItemModel) {
                                if (onThreeDotMenuItemClick != null){
                                    if (!TextUtils.isEmpty(track?.data?.misc?.movierights?.toString())){
                                        commonThreeDotsMenuItemModel.movieRights = track?.data?.misc?.movierights?.toString().toString()
                                    }
                                    commonThreeDotsMenuItemModel.forAdapter = forAdapter
