...
class RecentlyPlayViewModel : ViewModel() {
    ...
    fun getContinueWhereLeftListDetails(context: Context, bottomTabID: String): MutableLiveData<Resource<ContinueWhereLeftModel>>? {
        recentlyPlayRepos=RecentlyPlayRepos()
        return recentlyPlayRepos?.getDetailsFromStreamMultipleId(context,bottomTabID)
    }
}