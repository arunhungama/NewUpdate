...
abstract class BaseActivity : BaseServiceBoundedActivity(), View.OnClickListener,
    ...
    AnalyticsListener, AdLoader.AdLoaderListener, MediaPlayer.OnCompletionListener,
    ...
    var REQUEST_CODE_POST_NOTIFICATION_PERMISION = 106
    ...
    var mAdLoader = AdLoader()
    private var mAudioPlayer: MediaPlayer = MediaPlayer()
    var mAdRequestBuilder: AdRequestBuilder? = null
...
//        Utils.setStopScreenrecord(this@BaseActivity)
        ...
        updateDurationTask = object : Runnable {
            override fun run() {
                ...
                var isActivityVisible = HungamaMusicApp.getInstance().activityVisible
                if (isActivityVisible)
                {
                    isShareClicked = false
                }
                if (isShareClicked && !isActivityVisible)
                {
...
/*    fun fragmentonBackPressed(fm: FragmentManager): Boolean {
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        ...
        setLog("printRequestCode", " BaseActivity $requestCode")
...
//            cardImageCorner.alpha = 0f
                ...
                var artwork = ""
                 ...
                 if(track.movierights.let { CommonUtils.checkGoldMusic(it,track.contentType) }){
                    pausePlayer()
                    return@launch
                }
...
//        Itype50PagerAdapter.muteIconChange1()
...
/*        if (!CommonUtils.isUserHasGoldSubscription()){
            CommonUtils.openSubscriptionDialogPopupNew(applicationContext)
            return
        }*/
    ...
    private fun playableObserverResponse(it: PlayableContentModel) {
        ...
        if (it != null) {
            ...
            if (!TextUtils.isEmpty(it?.data?.head?.headData?.misc?.url)) {
                ...
                setLog(
                    "preCatchContent",
                    "BaseActivity-playableObserverResponse-foundUrl-track.id-${it.data.head.headData.id}"
                )
    ...
    fun callEventUserProperty(userProfileModel: UserProfileModel) {
        CoroutineScope(Dispatchers.IO).launch {
            ...
            if (userProfileModel.result == null || userProfileModel.result?.isEmpty() == true)
                return@launch
            ...
            if (!TextUtils.isEmpty(userProfileModel.result?.get(0)?.phone!!)) {
                userDataMap.put(EventConstant.PHONE, userProfileModel.result?.get(0)?.phone.toString())
            }
...
//                val age = currentYear?.toInt()!! - dobYear?.toInt()!!
            ...
            if (!TextUtils.isEmpty(userProfileModel.result?.get(0)?.phone))
                userDataMap.put(EventConstant.MOBILE_NUMBER, userProfileModel.result?.get(0)?.phone.toString())
            ...
            try {
                ...
                songDataList.forEachIndexed { index, song ->
                    if (song.id == playableContentModel.data.head.headData.id.toLong()) {
                        ...
                        setLog("sdthssdf", "${playableContentModel.data.head.headData.misc.movierights.toString()} ${song.id} ${song.title}")
                        if (!TextUtils.isEmpty(playableContentModel.data.head.headData.misc.movierights.toString()))
                        song.movierights = playableContentModel.data.head.headData.misc.movierights.toString()
                ...
                if (isPreCatchContent(songDataList, track)) {
                    ...
                    //callUserStreamUpdate(STREAM_POSITION_CURRENT)
                    val intent = Intent(this@BaseActivity, AudioPlayerService::class.java)
                       if(!CommonUtils.isUserHasGoldSubscription() && !BaseActivity.songDataList.isNullOrEmpty()
                    && BaseActivity.songDataList.size > BaseActivity.nowPlayingCurrentIndex()
                    && !BaseActivity.songDataList.get(BaseActivity.nowPlayingCurrentIndex()).movierights.contains("AMOD"))
                    intent.putExtra(Constant.IS_PAUSE, true)
                    else
...
//                callUserStreamUpdate()
    ...
    {
        try{
            if (track.playerType.equals("22", true) || track.playerType.equals("51", true) || track.playerType.equals("88888", true)){
                return
            }
            else {
                ...
                if(eventModel.bucketName.isEmpty())
                hashMap.put(EventConstant.BUCKETNAME_EPROPERTY, ""+Constant.title_heading)
...
//                var fistModel=list?.get(0)
                ...
                hashMap.put(EventConstant.GENRE_EPROPERTY, "" + eventModel?.genre)
                if(eventModel.label.isNotBlank()) hashMap.put(EventConstant.LABEL_EPROPERTY, "" + eventModel?.label)
                        ...
                        if (!downloadQueueList.isNullOrEmpty() && downloadQueueList.size > currentDownloadingIndex
                            ...
                            if (downloadQueueList.get(currentDownloadingIndex).downloadRetry >= MAX_DOWNLOAD_RETRY) {
                                ...
                                if (downloadQueueList.get(currentDownloadingIndex).contentType == ContentTypes.NONE.value
                                    ...
                                    || downloadQueueList.get(currentDownloadingIndex).contentType == ContentTypes.AUDIO_BOOK.value
                                    || downloadQueueList.get(currentDownloadingIndex).contentType == ContentTypes.AUDIO_STORY.value
                                ) {
                                    ...
                                    dpm.restrictedDownload = RestrictedDownload.NONE_DOWNLOAD_CONTENT
                                    if (userCanDownloadContent(this@BaseActivity, rootLayout, dpm, null,Constant.drawer_downloads_exhausted)) {
                                        val dwState = downloadTracker?.getDownloadState(downloadQueueList.get(currentDownloadingIndex).downloadUrl)
                                        ...
                                        if (dwState == -1 || dwState == androidx.media3.exoplayer.offline.Download.STATE_FAILED) {
                                            if (dwState == androidx.media3.exoplayer.offline.Download.STATE_FAILED) {
                                                removeVideoContentFromDownload(Uri.parse(downloadQueueList.get(currentDownloadingIndex).downloadUrl))
                                            }
                                            setLog("DownloadContent", "BaseActivity-callNextDownloadableItemFromQueue-3")
                                            setUpDownloadableContentListViewModel(downloadQueueList.get(currentDownloadingIndex).contentId!!, 5)
                                            ...
                                            val download = downloadTracker?.getDownload(downloadQueueList.get(currentDownloadingIndex).downloadUrl)
                                            if (download != null) {
            ...
            if (reason === Reason.DOWNLOAD_COMPLETED || reason === Reason.DOWNLOAD_CANCELLED
                ...
                if (reason === Reason.DOWNLOAD_COMPLETED) {
                    ...
                    if (downloadQueueList != null && (downloadQueueList.size > 0 && downloadQueueList.size > contentIndex)) {
                        ...
                        setLog("aglhdsalhg", "${data?.file.toString()}")
            ...
            when (download.state) {
                ...
                androidx.media3.exoplayer.offline.Download.STATE_COMPLETED -> {
                    ...
                    if (!downloadQueueList.isNullOrEmpty() && downloadQueueList.size > contentIndex) {
                        ...
                        setLog("aglhdsalhg", "${download.request.uri.toString()}")
...
//                        eventValue.put(AFInAppEventParameterName.CONTENT_TYPE, Utils?.getContentTypeName(""+downloadQueueList?.get(contentIndex).contentType))
    ...
    suspend fun getPlayableContentUrl(context: Context, id: String, contentType: Int) {
        if(!id.isNullOrBlank()) {
            ...
            setLog(
                "playbackQuality",
                "BaseActivity-getPlayableContentUrl-id-${id} - quality-$quality - isGoldUser-${getIsGoldUser()}"
            )
            val url =
                WSConstants.METHOD_PLAYABLE_CONTENT + id + "/url/playable?quality=" + quality + "&contentType=" + type + "&certificate=widevine" + if (type == 4) "&user=" + if (CommonUtils.isUserHasGoldSubscription()) "gold" else "free" else ""
                    ...
                    finalURL, Response.Listener { response ->
                        ...
                        CoroutineScope(Dispatchers.IO).launch {
                            try {
                                ...
                                if (playableContentModel.data.body.data.url.playable.size > 0) {
                                    ...
                                    if (TextUtils.isEmpty(playableContentModel.data.head.headData.misc.url) && playableContentModel.data.body.data.url.playable.size > 0) {
                                        ...
                                        if (BaseActivity.getIsGoldUser()) {
                                            var tmpList =
                                                playableContentModel.data.body.data.url.playable?.filter {
                                                    !it.key.equals("preview")
                                                }
                                            ...
                                            if (tmpList?.size!! > 0) {
                                                ...
                                                setLog(
                                                    TAG,
                                                    "getPlayableContentUrl: getIsGoldUser:${BaseActivity.getIsGoldUser()} quality url default first ser:${
                                                        tmpList.get(0).key
                                                    } "
                                                )
                                            } else {
                                ...
                                setLog(
                                    "playbackQuality",
                                    "BaseActivity-getPlayableContentUrl-End-id-${playableContentModel.data.head.headData.id}-title-${playableContentModel.data.head.headData.title} - key-${playableContentModel.data.head.headData.misc.urlKey} - isGoldUser-${getIsGoldUser()}"
                                )
                                ...
                                hashMap[EventConstant.RESPONSECODE_EPROPERTY] =
                                ...
                                hashMap[EventConstant.URL_EPROPERTY] =
        ...
        try {
            ...
            if (!isAudioAdPlaying && !isNextSongInProgress) {
                if (!isSleepTimerSetToEndOfCurrentPlay) {
                    ...
                    if (!songDataList.isNullOrEmpty() && songDataList?.size!! > nextIndex) {
                        updateNowPlayingCurrentIndex(nextIndex)
                       /* if (!CommonUtils.isUserHasGoldSubscription() && songDataList.get(nextIndex).movierights.contains("AMOD") == false)
                        {
                            pausePlayer()
                            return
                        }*/
                                    ...
                                    playContentOnline(songDataList, nowPlayingCurrentIndex(), false)
                                        ...
                                        if (audioPlayer != null && audioPlayer?.hasPrevious() == true) {
                                            ...
                                            HungamaMusicApp.getInstance()
                                                .setIsFirstLaunchSong(false)
                                            audioPlayer?.previous()
                                            playPlayer()
                                            isPause = false
                                            setLog(
                                                "preCatchApiCall",
                                                "BaseActitvity-playPreviousSong"
                                            )
                                            preCatchApiCall(
                                                songDataList,
                                                nowPlayingCurrentIndex(),
...
//                    callUserStreamUpdate(STREAM_POSITION_CURRENT)
    ...
    override fun onStoryPlatformClick(data: ShareStoryPlatformDialogModel) {
        ...
        if (data.title.equals("Facebook")) {
            ...
            data?.currentTrack?.title?.let {
                data?.currentTrack?.url?.let { it1 ->
                    data?.currentTrack?.image?.let { it2 ->
                        shareStoryImage(
...
//            isRecommendedListApiCalled = true
                            ...
                            if (it?.data != null) {
                                ...
                                if (!recommendedContentList.isNullOrEmpty()) {
                                    ...
                                    if (!songList.isNullOrEmpty()) {
                                        CoroutineScope(Dispatchers.IO).launch {
                                            delay(500)
                                            if (onLocalBroadcastEventCallBack != null){
                                                setLog("BroadcastReceiver", "mMessageReceiver-"+intent.getIntExtra("EVENT", 0))
                                                val intent = Intent(Constant.AUDIO_PLAYER_EVENT)
                                                intent.putExtra("EVENT", Constant.UPDATE_VERTICAL_ADAPTER)
                                                LocalBroadcastManager.getInstance(this@BaseActivity).sendBroadcast(intent)
                                            }
                                        }
                ...
                try {
                    if (!TextUtils.isEmpty(contentId) && !TextUtils.isEmpty(playerType)) {
                        try {
                            ...
                            if (totalDuration <= 0 && playedDuration <=0)
                            {}
                            else{
                                if (ConnectionUtil(this@BaseActivity).isOnline(false) && playerType == "5" || playerType == "22" || playerType == "114" || playerType == "117"){
    ...
    private fun updateDuration() {
        val intent = Intent(Constant.StopGameSound)
        LocalBroadcastManager.getInstance(this).sendBroadcast(intent)
        if (audioPlayer != null && audioPlayer?.isPlaying == true) {
            if (HungamaMusicApp.getInstance().activityVisible) {
                addFGTime()
...
//        }
    ...
    override fun gamificationOnPointsAdded(eventName: String?, added: Int, total: Int) {
        try {
            setLog(
                "GM-SDK-APP",
                "BaseActivity onPointsAdded: eventName:${eventName} added:${added}  total:${total} getPoint:${GamificationSDK.getPoints()}"
            )
            ...
            if (added > 0 && EventConstant.gamiFicationActions.contains(eventName?.trim())) {
                ...
                if (gcEventName != null && !TextUtils.isEmpty(gcEventName)) {
                    ...
                    if (gcEventModel.isDisplay!!) {
                        if (gcEventModel.popupType.contains("toast")) {
                            var msg = ""
                        ...
                        } else if (gcEventModel.popupType.contains("popup", true)) {
    ...
    override fun gamificationOnPointsUpdated(points: Int) {
        try {
            setLog("GM-SDK-APP", "BaseActivity onPointsUpdated:total:${points} getPoint:${GamificationSDK.getPoints()}")
            updateUserCoins()
        } catch (e: Exception) {
