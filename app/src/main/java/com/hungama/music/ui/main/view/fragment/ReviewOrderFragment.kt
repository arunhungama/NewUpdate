...
class ReviewOrderFragment(val product: ProductRespModel.Product) : BaseFragment() {
    ...
    private fun displayUserCoins(){
        setLog("redeemCoin", "displayUserCoins-ReviewOrderFragment-before-userCoins-$userCoin")
        userCoin = GamificationSDK.getPoints()
        setLog("redeemCoin", "displayUserCoins-ReviewOrderFragment-after-userCoins-$userCoin")
        ...
        btnOrderContinue.setOnClickListener {
            ...
            if (!isConfirmOrderClick){
                ...
                if (!TextUtils.isEmpty(shippingDetailModel.firstName)
                    ...
                    CommonUtils.showToast(requireContext(), messageModel,"ReviewOrderFragment","initializeComponent")
                }
            }else{
                callCreateOrderApi(product, shippingDetailModel)
            }
                    ...
                    Status.SUCCESS -> {
                        ...
                        if (it?.data?.status == true && it.data.message.equals("ok", true)) {
                            ...
                            if (!TextUtils.isEmpty(it?.data?.errors?.customer)) {
                                ...
                                CommonUtils.showToast(requireContext(), messageModel,"ReviewOrderFragment","callCreateOrderApi")
                                ...
                                CommonUtils.showToast(requireContext(), messageModel,"ReviewOrderFragment","getPlayableContentUrl")
                            }
                        }
                    }
    ...
    private fun getUserOrders() {
        ...
        if (ConnectionUtil(requireContext()).isOnline) {
            ...
            val messageModel = MessageModel(getString(R.string.toast_str_35), getString(R.string.toast_message_5),
                MessageType.NEGATIVE, true)
            CommonUtils.showToast(requireContext(), messageModel,"RadioFragment","getPlayableContentUrl")
        }
    }
