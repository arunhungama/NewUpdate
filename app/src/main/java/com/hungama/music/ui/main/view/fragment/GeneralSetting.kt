...
class GeneralSetting : BaseFragment() {
    ...
    override fun initializeComponent(view: View) {
        ...
        if (Build.VERSION.SDK_INT >= 33) {
            ...
            svEmailNotification.isChecked = true
            svMobileNotification.isChecked = true
            }
            else
            {
                svEmailNotification.isChecked = false
                svMobileNotification.isChecked = false
            }
    ...
    private fun fillUI(userSettingRespModel: UserSettingRespModel) {
        if (userSettingRespModel.data != null && userSettingRespModel.data?.data?.get(0)?.preference?.get(0) != null) {
            //            svEmailNotification.isChecked = userSettingRespModel.data?.data?.get(0)?.preference?.get(0)?.emailNotification!!
//            svMobileNotification.isChecked = userSettingRespModel.data?.data?.get(0)?.preference?.get(0)?.mobileNotification!!
...
//                            setProgressBarVisible(false)
            ...
            CommonUtils.showToast(requireContext(), messageModel,"GeneralSetting","getSetting")
        }
    }
    ...
    private fun saveSetting() {
        if (ConnectionUtil(requireContext()).isOnline) {
            ...
            val messageModel = MessageModel(getString(R.string.toast_str_35), getString(R.string.toast_message_5),
                MessageType.NEGATIVE, true)
            CommonUtils.showToast(requireContext(), messageModel,"GeneralSetting","saveSetting")
        }
    }
    ...
    private fun restartApp() {
        if (BaseActivity.updateDurationTask != null && BaseActivity.durationHandler != null){
            BaseActivity.durationHandler?.removeCallbacks(BaseActivity.updateDurationTask)
        }
        (activity as MainActivity).stopAudioPlayer()
        (activity as MainActivity).currentPlayer?.prepare()
