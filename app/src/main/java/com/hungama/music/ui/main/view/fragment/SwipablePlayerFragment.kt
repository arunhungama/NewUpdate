...
import com.hungama.music.eventanalytic.eventreporter.NudgeBannerClickedEvent
import com.hungama.music.eventanalytic.eventreporter.NudgeDrawerClickedEvent
import com.hungama.music.eventanalytic.eventreporter.NudgeDrawerView
...
class SwipablePlayerFragment : BaseFragment(), BaseActivity.OnLocalBroadcastEventCallBack, OnClickListener  {
    ...
    override fun initializeComponent(view: View) {
        ...
        if (!playerData.isNullOrEmpty()) {
            mainScope.launch {
                ...
                playerData?.let {
                    if (it.isNotEmpty()) {
                        if (currentViewPagerIndex < playerData?.size!!) {

                            if(playerData!![currentViewPagerIndex]?.contentType == ContentTypes.VIDEO.value){
                                (activity as BaseActivity).closeVideoMiniplayer()
                            }
    ...
    override fun onClick(v: View) {
        ...
        when(v.id){
            ...
            R.id.llBuyPlanNewPreview,R.id.llBuyPlanExhausted,R.id.llBuyPlanPreview->{
                ...
                isClickedPay = true
                eventClickFunction(CommonUtils.getNudgePlanId())
            }
    ...
    fun eventClickFunction(planId: String) {
        setLog("khbdksahg", "swip isClicked")
        val hashMapPageView = HashMap<String, String>()
        hashMapPageView[EventConstant.TYPE_EPROPERTY] = "drawer_minute_quota_exhausted"
        hashMapPageView[EventConstant.SCREEN_NAME_EPROPERTY] = "player_music"
        ...
        if (planId.isNotEmpty()) hashMapPageView[EventConstant.button_text] = button_text
        else hashMapPageView[EventConstant.button_text] = "See All"
    ...
    fun callPayApi(url:String) {
        if (ConnectionUtil(activity).isOnline) {
            ...
            CommonUtils.showToast(requireContext(), messageModel,"SwipablePlayerFragment","callPayApi")
        }
    }
...
//        url += "&upilist=${Utils.convertArrayToString(upiList)}"
    ...
    override fun onLocalBroadcastEventCallBack(context: Context?, intent: Intent) {
        if (isAdded) {
                    ...
                    mainScope.launch {
                        ...
                        if (!playerData.isNullOrEmpty() && currentViewIndex != null && playerData?.size!! > currentViewIndex && playerData?.get(currentViewIndex)?.contentType == ContentTypes.Audio_Ad.value
                            ...
                            if (currentViewIndex != BaseActivity.nowPlayingCurrentIndex()){
                                if (!playerData.isNullOrEmpty() && !BaseActivity.songDataList.isNullOrEmpty() && playerData?.size != BaseActivity.songDataList?.size){
                                    playerData = BaseActivity.songDataList
                                    storiesPagerAdapter = SwipablePlayerPagerAdapter(this@SwipablePlayerFragment, playerData!!)
                                    view_pager_stories?.adapter = storiesPagerAdapter
            ...
            else if (event == Constant.UPDATE_VERTICAL_ADAPTER){
//                playerData = BaseActivity.songDataList
                setLog("SwipablePlayerFragment1", "Updating ${playerData?.size}")
//                   if (storiesPagerAdapter != null){
                       //view_pager_stories?.isUserInputEnabled = true
                       //storiesPagerAdapter.updateDataList(playerData!!)
