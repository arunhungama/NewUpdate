...
import kotlinx.android.synthetic.main.shimmer_layout.viewHero
...
class DiscoverTabFragment : BaseFragment(), OnParentItemClickListener, TracksContract.View, BucketParentAdapter.OnMoreItemClick,
    ...
    BaseActivity.OnDownloadVideoQueueItemChanged, RetryApiInterface {
    ...
    var flag = true
    var isMore = false
    lateinit var onMoreClick: BucketParentAdapter.OnMoreItemClick
    ...
    override fun initializeComponent(view: View) {
        ...
        baseMainScope.launch {
            if (arguments!=null){
                ...
                if (arguments?.containsKey(Constant.IS_MORE)!!){
                    isMore = arguments?.getBoolean(Constant.IS_MORE)!!
                }
            }
...
//                dailyDoseAPICall(DiscoverMainTabFragment.mHomeModel)
            ...
            if (!isMore && isCategoryPage && !TextUtils.isEmpty(categoryName) && !TextUtils.isEmpty(categoryId)){
                val bundle = Bundle()
...
//                        delay(3000)
                ...
                if (isMore){
                    for(i in 0 until bucketRespModel?.data?.body?.rows?.size!!){
                        if (categoryId == bucketRespModel?.data?.body?.rows?.get(i)?.id){
                            onMoreClick.onMoreClick(bucketRespModel?.data?.body?.rows?.get(i), i)
                            arguments?.putString(Constant.EXTRA_CATEGORY_NAME, "")
                            arguments?.putBoolean(Constant.IS_MORE, false)
                        }
                    }
                }
...
//                        }
                        ...
                        BucketParentAdapter.isVisible = MainActivity.headerItemPosition == 0 && completeVisiblePosition <= 0
...
//===================================================================================
    ...
    override fun onParentItemClick(parent: RowsItem, parentPosition: Int, childPosition: Int) {
        ...
        if (parent.items!!.get(childPosition)?.data?.type!!.equals("21",true)||parent.items!!.get(childPosition)?.data?.type!!.equals("110",true)||parent.items!!.get(childPosition)?.data?.type!!.equals("77777",true)||parent.items!!.get(childPosition)?.data?.type!!.equals("34",true) || parent.items!!.get(childPosition)?.data?.type!!.equals("114", true) || parent.items!!.get(childPosition)?.data?.type!!.equals("117", true)) {
            ...
            else if(parent.items!!.get(childPosition)?.data?.type!!.equals("114",true)){
                CONTENT_TYPE = Constant.CONTENT_AUDIO_BOOK
            }
            else if(parent.items!!.get(childPosition)?.data?.type!!.equals("117",true)){
                CONTENT_TYPE = Constant.CONTENT_AUDIO_STORY
            }
            if (!CommonUtils.isUserHasGoldSubscription())
            {
                if((CONTENT_TYPE == Constant.CONTENT_AUDIO_BOOK || CONTENT_TYPE == Constant.CONTENT_AUDIO_STORY)  && parent.items!!.get(childPosition)?.data?.misc?.movierights?.contains("AMOD") == true){
                    //Empty
                }
                else if (CONTENT_TYPE == Constant.CONTENT_PODCAST && CommonUtils.checkMovieRight(parent.items!!.get(childPosition)?.data?.movierights as MutableList)
                        && CONTENT_TYPE == Constant.CONTENT_MUSIC && CommonUtils.checkMovieRight(parent.items!!.get(childPosition)?.data?.misc?.movierights  as MutableList)){
                    // empty
                }
            ...
            if (parent.itype == 50) {
                ...
                EventButtonClickFunction("",childPosition, parent?.items?.get(childPosition)?.data)
    ...
    override fun bannerItemClick(
        ...
        if (ConnectionUtil(context).isOnline) {
            ...
            EventButtonClickFunction("secondaryCta",position, bodyData?.data)
    ...
    fun EventButtonClickFunction(type:String,position: Int,data: BodyDataItem?) {
        val hashMapPageView = HashMap<String, String>()
        if(type.equals("secondaryCta")){
            hashMapPageView[EventConstant.secondary_cta_option_selected] = data?.secondaryCta.toString()
        }else {
            hashMapPageView[EventConstant.primary_cta_option_selected] = data?.primaryCta?.id.toString()
        }
...
//                dpm.queryParam = queryParam
                ...
                if (CommonUtils.userCanDownloadContent(requireContext(), rlMain, dpm, this@DiscoverTabFragment,Constant.drawer_svod_download,
                        CommonUtils.getFirebaseConfigAdsData()?.drawerSvodDownload?.let { CommonUtils.checkEnableFlag(it) }.toString())) {
        ...
        baseIOScope.launch {
            ...
            if (seasonList.isNotEmpty()) {
                ...
                if (!CommonUtils.checkUserCensorRating(
                    ...
                    val dpm1 = DownloadPlayCheckModel()
                    dpm1.contentId = seasonList.get(0).data.id
                    dpm1.planName =  seasonList.get(0).data.misc.movierights.toString()
                    dpm1.isAudio = true
                    dpm1.isDownloadAction = true
                    dpm1.isShowSubscriptionPopup = true
...
//        }
        ...
        if (playableItem.items?.get(position)?.data?.misc?.movierights != null){
            track.movierights = playableItem.items?.get(position)?.data?.misc?.movierights.toString()
        }
...
/*        bucketParentAdapter?.pauseOriginalPlayer()
    ...
    override fun retryApi(url: String?) {
