...
import com.hungama.music.ui.main.view.activity.LoginMainActivity.FacebookFields
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        receiver = object : BroadcastReceiver() {
            override fun onReceive(context: Context?, intent: Intent?) {
                CommonUtils.showActivity(this@PaymentWebViewActivity, PaymentWebViewActivity::class.java)
            }
        }
        val lbm = LocalBroadcastManager.getInstance(this)
        lbm.registerReceiver(receiver as BroadcastReceiver, IntentFilter(Constant.PAYMENT_WEB))
...
//                    }else upiPayIntent.data = request?.url
                    ...
                    if (null != intent.resolveActivity(packageManager)) {
                        ...
                        val messageModel = MessageModel(
                            getString(R.string.payment_str_5),
                            MessageType.NEUTRAL,
                            true
                        )
                        CommonUtils.showToast(this@PaymentWebViewActivity, messageModel)
                    ...
                    return true
                        ...
                        .contains("https://payments.hungama.com/payment_success") && !isPaymentSuccess
                ) {
...
//                }
                        ...
                        .contains("/payment/success/") || request?.url.toString()
                    ...
                    if (args != null && args.size > 0) {
                        args?.forEach {
                            if (request?.url != null && it != null && request?.url?.getQueryParameter(
                                    it
                                ) != null
                            ) {
                                googleBillingPaysplitArray.put(
                                    it,
                    ...
                    if (request?.url != null) {
                        ...
                        setLog("WEbViewUrl", " shouldOverrideUrlLoading ${request?.url.toString()}")
                    }
                    setLog(
                        TAG,
                        "GoogleBillingPayCall step 1: isGoogleBillingPayCall->${isGoogleBillingPayCall} googleBillingPaysplitArray:${googleBillingPaysplitArray}"
                    )
                    ...
                    setLog(
                        TAG,
                        "GoogleBillingPayCall step 2: isGoogleBillingPayCall->${isGoogleBillingPayCall} googleBillingPaysplitArray:${googleBillingPaysplitArray}"
                    )
                    if (isGoogleBillingPayCall) {
                        ...
                        if (args != null && args.size > 0) {
                            ...
                            setLog(
                                TAG,
                                "GoogleBillingPayCall gpOrdersplitArray->" + gpOrdersplitArray
                            )
            ...
            override fun onLoadResource(view: WebView?, url: String?) {
                ...
                if (domainUrl.contains("hungama.com/payment") || domainUrl.contains("hungama.com/plan")) {
                    ...
                    if (view.progress == 100 && !eventlastWebURL.equals(domainUrl)) {
                        setLog(
                            TAG,
                            "onLoadResource: hungama.com/payment callWebLoadEvent :${domainUrl} progress:${view?.progress}"
                        )
            ...
            override fun onPageStarted(view: WebView?, url: String?, favicon: Bitmap?) {
                ...
                currentWebURL = view?.url?.let { CommonUtils.getUrlWithoutParameters(it) }.toString()
                requestTime = DateUtils.getCurrentDateTime()
            ...
            override fun onReceivedError(
                ...
                if (view.progress == 100 && !eventlastWebURL.equals(domainUrl)) {
                    setLog(
                        TAG,
                        "onPageFinished: lastWebURL:${domainUrl} progress:${view?.progress}"
                    )
            ...
            fun callWebLoadEvent(url: String, errorCode: Int, description: String) {
                CoroutineScope(Dispatchers.IO).launch {
                    ...
                    if (paySplitArray != null && paySplitArray.containsKey("content_id")) {
                        if (!TextUtils.isEmpty(
                                HungamaMusicApp.getInstance()
                                    .getEventData(paySplitArray?.get("content_id")!!).sourceName
                            )
                        ) {
                            hashMap.put(
                    ...
                    hashMap.put(
                        EventConstant.ACTION_EPROPERTY,
                        "" + paySplitArray?.get("content_id")
                    )
                    ...
                    hashMap.put(
                        ...
                        "" + ConnectionUtil(this@PaymentWebViewActivity).networkType
                    )
        ...
        if (!TextUtils.isEmpty(url)) {
            ...
            if (args != null && args.size > 0) {
                args.forEach {
                    if (url?.toUri() != null && it != null && url?.toUri()
                            ?.getQueryParameter(it) != null
                    )
...
//    private val LICENSE_KEY: String? = ""
    ...
    private val LICENSE_KEY: String =
        ""
...
//                    CommonUtils.showToast(this@PaymentWebViewActivity, messageModel)
                        ...
                        if (bp != null && bp?.isConnected!! && productIdsList.size > 0) {
                            ...
                            if (googleBillingPaysplitArray.containsKey("plan_type")) {
                                ...
                                if (googleBillingPaysplitArray.get("plan_type")
                                    ...
                                    details =
                                        bp?.getSubscriptionPurchaseInfo(productIdsList?.get("store_payment_id"))!!
                                } else {
                                    details =
                                        bp?.getPurchaseInfo(productIdsList?.get("store_payment_id"))!!
                                }
                            }
    ...
    private fun callInApp() {
        if (bp != null && bp?.isConnected!! && productIdsList.containsKey("store_payment_id") && googleBillingPaysplitArray.containsKey(
                "plan_type"
            )
        ) {
            showToast(
    ...
    private fun updatePurchaseItem(purchaseInfo: PurchaseInfo?) {
        ...
        if (purchaseInfo?.purchaseData?.purchaseToken != null && gpOrdersplitArray.containsKey("store_payment_id") && googleBillingPaysplitArray.containsKey(
                "plan_type"
            )
        ) {
            ...
            if (googleBillingPaysplitArray.get("plan_type")?.contains("subscription", true)!!
                ...
                setLog(
                    TAG,
                    "updatePurchaseItem: product_id:${gpOrdersplitArray?.get("store_payment_id")}"
                )
                ...
                when (it.status) {
                    Status.SUCCESS -> {
                        ...
                        if (it?.data != null && it?.data?.success!!) {
                            ...
                            val diff: Long = TimeUnit.MILLISECONDS.toMillis(diffInMillies)
                            val planName =
                                if (intent.hasExtra("planName")) intent.getStringExtra("planName") else ""
                            val contentId =
                                if (intent.hasExtra("contentId")) intent.getStringExtra("contentId") else ""
...
//                                                        EventManager.getInstance().sendEvent(AF_Subscribe(hashMapCheck))
                                                    ...
                                                    EventManager.getInstance().sendEvent(
                                                        SubscriptionSuccessfulPageEvent(hashMap)
                                                    )
                                                    ...
                                                    EventManager.getInstance()
                                                        .sendEvent(RentPaymentOptionEvent(hashMap1))
                                                }
                                            }
...
//                                                AppsFlyerLib.getInstance().logEvent(HungamaMusicApp.getInstance().applicationContext!!,EventConstant.AF_SUBSCRIBE_ENAME, eventValue)
                                            ...
                                            val hashMapCheck = HashMap<String, String>()
                                            hashMapCheck[AFInAppEventParameterName.REVENUE] =
                                                it?.data?.data?.subscription?.planPrice!!
                                            hashMapCheck[AFInAppEventParameterName.CURRENCY] =
                                                it?.data?.data?.subscription?.currency!!
                                            hashMapCheck.put(
                                                EventConstant.AF_START_DATE_PROPERTY,
                                                it?.data?.data?.subscription?.subscriptionStartDate!!
...
//                                                EventManager.getInstance().sendEvent(AF_Subscribe(hashMapCheck))
                                            ...
                                            EventManager.getInstance()
                                                .sendEvent(SubscriptionSuccessfulPageEvent(hashMap))
...
//                                                setLog("TAG", "AppsFlyerLib eventName:${EventConstant.AF_SUBSCRIBE_ENAME} eventProperties:${eventValue}")
            ...
            val messageModel = MessageModel(
                getString(R.string.discover_str_3), getString(R.string.toast_message_5),
