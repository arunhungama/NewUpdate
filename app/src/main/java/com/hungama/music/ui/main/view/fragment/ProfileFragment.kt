...
import kotlinx.android.synthetic.main.fragment_profile.tvCoinCount
...
class ProfileFragment : BaseFragment(), LogoutDialog.LogoutListener, OnUserSubscriptionUpdate {
    ...
    private fun displayUserCoins(){
        ...
        try {
            if(isAdded()){
                userCoins = GamificationSDK.getPoints()
...
/*        if (SharedPrefHelper.getInstance().isUserLoggedIn())
    ...
    override fun initializeComponent(view: View) {
        ...
        if (SharedPrefHelper.getInstance().isUserLoggedIn()){
            ...
            setLog("initializedGamification", " Profile " +gmfSDKCoins.toString())
        }
    ...
    override fun onDestroy() {
        ...
        showBottomNavigationAndMiniplayer()
        if (dash_gold != null) {
            context?.let { CommonUtils.setGoldButton(it, dash_gold) }
        }
        super.onDestroy()
    }
    ...
    private fun setUpViewModel() {
        ...
        if (isAdded) {
            userSubscriptionViewModel =
                ViewModelProvider(this).get(UserSubscriptionViewModel::class.java)
    ...
    fun getUserSubscriptionStatus() {
        if (ConnectionUtil(requireContext()).isOnline) {
            ...
            CommonUtils.showToast(requireContext(), messageModel,"ProfileFragment","getUserSubscriptionStatus")
        }
    }
    ...
    private fun fillUI(userSubscriptionModel: UserSubscriptionModel) {
        if (userSubscriptionModel != null && userSubscriptionModel?.data != null && userSubscriptionModel?.data?.status!!) {
            ...
            if(userSubscriptionModel?.data?.subscription!=null){
                ...
                userDataMap.put(EventConstant.SUBSCRIPTION_STATUS_EPROPERTY, ""+ userSubscriptionModel?.data?.user?.userMembershipType)

                val strDate = DateUtils.convertDate(
                    DateUtils.DATE_FORMAT_YYYY_MM_DD_HH_MM_SS,
                    DateUtils.DATE_FORMAT_YYYY_MM_DD,
                    "" + userSubscriptionModel?.data?.subscription?.subscriptionStartDate
                )
                val endDate = DateUtils.convertDate(
                    DateUtils.DATE_FORMAT_YYYY_MM_DD_HH_MM_SS,
                    DateUtils.DATE_FORMAT_YYYY_MM_DD,
                    "" + userSubscriptionModel?.data?.subscription?.subscriptionEndDate
                )
                userDataMap.put(EventConstant.SUBSCRIPTION_START_DATE, ""+ strDate)
                userDataMap.put(EventConstant.SUBSCRIPTION_END_DATE, ""+ endDate)
                ...
                userDataMap.put(EventConstant.SUBSCRIPTION_START_DATE_TIME, ""+ userSubscriptionModel?.data?.subscription?.subscriptionStartDate)
                userDataMap.put(EventConstant.SUBSCRIPTION_END_DATE_TIME, ""+ userSubscriptionModel?.data?.subscription?.subscriptionEndDate)
...
//                )
    ...
    private fun getUserProfile() {
        ...
        if (isAdded) {
            ...
            if (ConnectionUtil(requireContext()).isOnline && userViewModel != null) {
                ...
                CommonUtils.showToast(requireContext(), messageModel,"ProfileFragment","getUserProfile")
            }
        }
    }
                    ...
                    Observer {
                        when(it.status){
                            ...
                            Status.ERROR ->{
                                ...
                                CommonUtils.showToast(requireContext(), messageModel,"ProfileFragment","callSocialLogin")
    ...
    private fun signInToFacebook() {
        setProgressBarVisible(true)
        LoginManager.getInstance().logInWithReadPermissions(requireActivity(), listOf("public_profile", "user_friends", "email"))
    }
...
// Initialize Facebook Login button
        ...
        LoginManager.getInstance().registerCallback(callbackManager, object :
            ...
            override fun onCancel() {
                ...
                CommonUtils.showToast(requireContext(), messageModel,"ProfileFragment","configureFacebook")
            }
...
//            }
    ...
    override fun LogoutUser() {
        ...
        setLog("GM-SDK-APP","before LogoutUser called isUserLoggedIn:${isUserLoggedIn} getPoints:${GamificationSDK.getPoints()}")

        GamificationSDK.deInitialize(requireContext())
        setLog("GM-SDK-APP","after LogoutUser called isUserLoggedIn:${isUserLoggedIn} getPoints:${GamificationSDK.getPoints()}")
        ...
        if (pending != null) {
            ...
            auth.startActivityForSignInWithProvider(requireActivity(), provider.build())
                ...
                .addOnFailureListener { e ->
                    ...
                    CommonUtils.showToast(requireContext(), messageModel,"ProfileFragment","signInToApple")
                }
        }
    }
