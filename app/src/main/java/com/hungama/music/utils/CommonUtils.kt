...
import com.hungama.music.eventanalytic.eventreporter.ToastEvent
...
object CommonUtils {
    ...
    private var paymentApps = HashMap<String, String>()
    ...
    fun showActivity(context:Context, cls: Class<*>?){
            ...
            val startMain = Intent(context, cls)
                    ...
                    try {
                        ...
                        if (status && bitmap != null) {
                            Palette.from(bitmap).generate { palette ->
                                ...
                                color = palette!!.getLightMutedColor(R.attr.colorPrimaryDark)
...
//                    rootLayout.setBackground(getGradientDrawable(getTopColor(palette), getCenterLightColor(palette), getBottomDarkColor(palette)))
    ...
    fun setArtImageDarkBg1(artImageUrl: String, view1: ImageView, view2: ImageView, view3: ImageView): Int {

        var color: Int = 0

        if (artImageUrl != null && !TextUtils.isEmpty(artImageUrl) && URLUtil.isValidUrl(artImageUrl) && view1 != null && view2 != null && view3 != null) {
            try {
                setLog(TAG, "setArtImageDarkBg: $artImageUrl")
                val result: Deferred<Bitmap?> = GlobalScope.async {
                    val urlImage = URL(artImageUrl)
                    ...
                    try {
                        ...
                        if (bitmap != null) {
                            Palette.from(bitmap).generate { palette ->
                                ...
                                color = calculateAverageColor(bitmap, 1)
                                view1.setColorFilter(lightenColor( color, 0.8f), PorterDuff.Mode.SRC_ATOP)
                                view2.setColorFilter(lightenColor( color, 0.9f), PorterDuff.Mode.SRC_ATOP)
                                view3.setColorFilter(lightenColor(color, 1f), PorterDuff.Mode.SRC_ATOP)

/*                                view1.setColorFilter(Color.parseColor("#af7712"),android.graphics.PorterDuff.Mode.MULTIPLY)
                                view2.setColorFilter(Color.parseColor("#DB9619"), android.graphics.PorterDuff.Mode.MULTIPLY)
                                view3.setColorFilter(Color.parseColor("#EAAC3D"),android.graphics.PorterDuff.Mode.MULTIPLY)*/
                            }
                        }
                    } catch (exp: Exception) {
                        exp.printStackTrace()
...
/*    fun lighterHex(color:Int):String{
        val hex = Integer.toHexString(4001)
        assertEquals("fa1", hex)
    }*/
    fun lightenColor(color: Int, factor: Float): Int {
        val alpha = Math.round(Color.alpha(color) * factor)
        val red = Math.round(Color.red(color) * factor)
        val green = Math.round(Color.green(color) * factor)
        val blue = Math.round(Color.blue(color) * factor)
                ...
                GlobalScope.launch(Dispatchers.IO) {
                    try {
                        ...
                        if (status && bitmap != null) {
                            Palette.from(bitmap).generate { palette ->
                                //color = palette!!.getMutedColor(R.attr.colorPrimary)
                                color = palette!!.getMutedColor(R.attr.colorPrimaryDark)
...
//        if (!isGoldUser) streamQualityModel3.isSelected = true
            ...
            CoroutineScope(Dispatchers.IO).launch {
                ...
                if (it.statusCode == 200 && it.result != null && it.result?.size!! > 0) {
                    ...
                    if (!TextUtils.isEmpty(it.result?.get(0)?.phone)) {
                        SharedPrefHelper.getInstance().save(PrefConstant.USER_MOBILE, it.result?.get(0)?.phone)
                        userDataMap.put(EventConstant.MOBILE_NUMBER , it.result?.get(0)?.phone.toString())
        ...
        if (isUserHasGoldSubscription() && (planType == PlanTypes.SUBSCRIPTION.value || planType == PlanTypes.FREE.value)) {
            ...
            if (planTxtName.contains(PlanNames.SVOD.name, true)) {
                openSubscriptionDialogPopup(
                    ...
                    onUserSubscriptionUpdateCall,
                    downloadPlayCheck.contentTitle,
                    downloadPlayCheck, trigger_point,banner_type
                )
                return false
            ...
            if (allDownloadItemList != null) {
                if (allDownloadItemList.size >= getAvailableDownloadContentSize(context)) {
                    ...
                    openSubscriptionDialogPopup(
                        ...
                        downloadPlayCheck.contentTitle,
                        downloadPlayCheck,
                        Constant.drawer_downloads_exhausted,banner_type
                    )
    ...
    fun openSubscriptionDialogPopup(
        ...
        trigger_point: String = "",
        banner_type: String = "",
        onUserSubscriptionDialogIsFinish: OnUserSubscriptionDialogIsFinish? = null,country_check: String = "") {
        if (isShowSubscriptionPopup) {
            try {
                val subscriptionDialogBottomsheetFragment = SubscriptionDialogBottomsheetFragment(
                    ...
                    downloadPlayCheck,
                    trigger_point,
                    banner_type,country_check, onUserSubscriptionDialogIsFinish
                )
    ...
    fun openSubscriptionDialogPopupNew(context: Context) {
        CommonUtils.openSubscriptionDialogPopup(context,
            PlanNames.SVOD.name, "", true,
            null, "", null,Constant.drawer_genric1_subscription)
...
//        }
    ...
    fun applyAppLogo(context: Context, view: AppCompatImageView) {
        MainScope().launch {
            if (isUserHasGoldSubscription()) {
                ...
                view.setImageResource(R.drawable.gold_subscription_detail_bg)
                ...
                view.setImageResource(R.drawable.hungama_text_icon_new)
    ...
    fun getDeeplinkIntentData(appLinkData: Uri): Intent = runBlocking(Dispatchers.IO) {
        ...
        if (!pathArray.isNullOrEmpty()) {
            ...
            if (pathArray.size > 5) {
                ...
                if (pageName == "audiobook" || pageName == "audiostory"){
                    isEpisode = true
                    episodeId = pathArray[5]
                }
            }
            if (pathArray.size > 6) {
    ...
    fun getNudgeAudioId(): String {
        var audioIds = ""
            if (isUserHasEliableFreeContent()) {
                ...
                if (firAudioId != null) {
                    audioIds = getValueFromCurrentLang(firAudioId.toString())
                }
            } else {
                val firAudioId =
    ...
    private fun callStreamFailedEvent(error: PlaybackException) {
        val currentIndex = BaseActivity.nowPlayingCurrentIndex()
        if (!BaseActivity.songDataList.isNullOrEmpty() && BaseActivity.songDataList?.size!! > currentIndex) {
            val hashMap = HashMap<String, String>()
            val mSongTrack = BaseActivity.songDataList?.get(currentIndex)
            ...
            BaseActivity.streamName = "streamFailed"
        }
    ...
    fun getSongDurationConfig(): SongDurationConfigModel = runBlocking(Dispatchers.IO) {
        ...
        if (nudge_stream_preview.isNotEmpty()) {
            val jsonObjec_nudge_stream_preview = JSONObject(nudge_stream_preview)
            durationConfigModel.nudge_stream_preview = Gson().fromJson(
                jsonObjec_nudge_stream_preview.toString(),
        ...
        durationConfigModel.global_limited_minutes_quota = if (!global_limited_minutes_quota.isNullOrEmpty()) global_limited_minutes_quota.toInt() else 0
        ...
        if (drawer_minute_quota_exhausted.isNotEmpty()) {
        ...
        if (nudge_minute_quota_exhausted.isNotEmpty()) {
        ...
        setLog("printSongDuration", durationConfigModel.toString())
        durationConfigModel
    }

    fun getValueFromCurrentLang(str: String): String {
        val audioId = ""
        ...
        try {
            ...
            setLog("objectDataFromeFirebase", "exception ${e.message}")
        }
        return audioId
    }

    fun getValueFromCurrentLang1(str: String): String {
        val audioId = ""
        val map: MutableMap<String, String> = HashMap()
        var sIterator: Iterator<String>? = null
        ...
        try {
            ...
            while (sIterator.hasNext()) {
                ...
                val key = sIterator.next()
                val value: String = JSONObject(str).getString(key)
                setLog("objectDataFromeFirebase", "loop $value")
                map[key] = value
            ...
            if (!map.isNullOrEmpty()) {
                val currentLang = SharedPrefHelper.getInstance().getLanguage()
                setLog("objectDataFromeFirebase "," map " + currentLang + " " + map.toString()+ " " + map[currentLang].toString())
                return map[currentLang].toString().replace("_"," ")
            }
        } catch (e: JSONException) {
            setLog("objectDataFromeFirebase", "exception ${e.message}")
        }
    ...
    fun getFirebaseConfigAdsData(): AdsConfigModel = runBlocking(Dispatchers.IO) {
        ...
        if (timeDiff < 10 && adsConfigModel != null) {
            ...
            try {
                ...
                val drawer_streaming_podcast = remoteConfig.getString("drawer_streaming_podcast")
                val drawer_genric1_subscription = remoteConfig.getString("drawer_genric1_subscription")
                val drawer_streaming_mymix = remoteConfig.getString("drawer_streaming_mymix")
                val drawer_streaming_sleeptimer = remoteConfig.getString("drawer_streaming_sleeptimer")
                ...
                val nudge_music_video_banner = remoteConfig.getString("nudge_music_video_banner")
                val video_prompt = remoteConfig.getString("video_prompt")
                val drawer_streaming_audiobook = remoteConfig.getString("drawer_streaming_audiobook")

                val ribbon_pdc_gold = remoteConfig.getBoolean("ribbon_pdc_gold") ?: false
                val ribbon_pdc_free = remoteConfig.getBoolean("ribbon_pdc_free") ?: false
                val ribbon_pdc_limited = remoteConfig.getBoolean("ribbon_pdc_limited") ?: false

                val ribbon_audio_book_free = remoteConfig.getBoolean("ribbon_audio_book_free") ?: false
                val ribbon_audio_book_gold = remoteConfig.getBoolean("ribbon_audio_book_gold") ?: false
                val ribbon_audio_book_limited = remoteConfig.getBoolean("ribbon_audio_book_limited") ?: false

                val ribbon_audio_stories_free = remoteConfig.getBoolean("ribbon_audio_stories_free") ?: false
                val ribbon_audio_stories_gold = remoteConfig.getBoolean("ribbon_audio_stories_gold") ?: false
                val ribbon_audio_stories_limited = remoteConfig.getBoolean("ribbon_audio_stories_limited") ?: false

                val mi_deeplink_sessions = remoteConfig.getString("mi_deeplink_sessions")
                ...
                fConfigJsonObject.add(
                    "drawer_streaming_podcast",
                    convertStringToJsonObject(drawer_streaming_podcast)
                )
                fConfigJsonObject.add(
                    "drawer_genric1_subscription",
                    convertStringToJsonObject(drawer_genric1_subscription)
                )
                fConfigJsonObject.add(
                    "drawer_streaming_sleeptimer",
                    convertStringToJsonObject(drawer_streaming_sleeptimer)
                )

                fConfigJsonObject.add(
                    "drawer_streaming_mymix",
                    convertStringToJsonObject(drawer_streaming_mymix)
                )
                ...
                fConfigJsonObject.add(
                    "mi_deeplink_sessions",
                    convertStringToJsonObject(mi_deeplink_sessions)
                )
                ...
                fConfigJsonObject.add("nudge_music_video_banner", convertStringToJsonObject(nudge_music_video_banner))

                fConfigJsonObject.add("video_prompt", convertStringToJsonObject(video_prompt))
                fConfigJsonObject.addProperty("ribbon_pdc_free", ribbon_pdc_free)
                fConfigJsonObject.addProperty("ribbon_pdc_gold", ribbon_pdc_gold)
                fConfigJsonObject.addProperty("ribbon_pdc_limited", ribbon_pdc_limited)

                fConfigJsonObject.addProperty("ribbon_audio_book_free", ribbon_audio_book_free)
                fConfigJsonObject.addProperty("ribbon_audio_book_gold", ribbon_audio_book_gold)
                fConfigJsonObject.addProperty("ribbon_audio_book_limited", ribbon_audio_book_limited)

                fConfigJsonObject.addProperty("ribbon_audio_stories_free", ribbon_audio_stories_free)
                fConfigJsonObject.addProperty("ribbon_audio_stories_gold", ribbon_audio_stories_gold)
                fConfigJsonObject.addProperty("ribbon_audio_stories_limited", ribbon_audio_stories_limited)
                fConfigJsonObject.add("drawer_streaming_audiobook", convertStringToJsonObject(drawer_streaming_audiobook))
            ...
            setLog("FConfigObjectMain", Gson().toJson(adsConfigModel.toString()))
            adsConfigModel as AdsConfigModel
        }
    }
    ...
    fun getMiSession(): Int {
        return getFirebaseConfigAdsData().miDeeplinkSessions.till_session
    }

    fun getMiDeeplink(): String {
        return getFirebaseConfigAdsData().miDeeplinkSessions.deep_link_url
    }
    ...
    fun setPageBottomSpacing(
        ...
        CoroutineScope(Dispatchers.Main).launch {
            try {
                ...
                setLog("setPageBottomSpacing", "CommonUtils-mTopPadding: $mTopPadding $bottomPadding")
        ...
        try {
            ...
            if (!TextUtils.isEmpty(messageModel.message)){
                ...
                if (messageModel.messageType == MessageType.NEGATIVE){
                    ...
                    val view1 = layout.findViewById<ImageView>(R.id.view1)
                    val colors = intArrayOf(Color.parseColor("#FFE21E3E"),
                        Color.parseColor("#FFF72C3A"))
                    band_color="red"
                    val position = floatArrayOf(
                        0f,
                        1f
                    ...
                    if (context is AppCompatActivity){
                        ...
                        val inflater =
                            context.getSystemService(Context.LAYOUT_INFLATER_SERVICE) as LayoutInflater
                        layout =
                            inflater.inflate(R.layout.toast_green, null)
                        //val view = layout.findViewById(R.id.toast_container) as ConstraintLayout
                ...
                if(!messageModel.message.contains(context.getString(R.string.toast_message_5))) {
                    ...
                    hashMap.put(EventConstant.BANDCOLOUR_EPROPERTY, "" + band_color)
                    hashMap.put(EventConstant.PAGE_NAME_EPROPERTY, "" + page_name)
                    hashMap.put(EventConstant.STRINGVALUE_EPROPERTY, "" + messageModel?.message)
                    hashMap.put(EventConstant.APINAME_EPROPERTY, "" + api_name)
                    hashMap.put(
                        EventConstant.NETWORKTYPE_EPROPERTY,
                        ConnectionUtil(context).networkType
                    )
                    hashMap.put(EventConstant.ERRORCODE_EPROPERTY, "" + error_code)
                    hashMap.put(EventConstant.RESPONSECODE_EPROPERTY, "" + response_code)
                    hashMap.put(EventConstant.RESPONSETIME_EPROPERTY, "" + response_time)
                    hashMap.put(EventConstant.URL_EPROPERTY, "" + url)

                    EventManager.getInstance().sendEvent(ToastEvent(hashMap))
                    setLog("Hashmap", "hashevents${hashMap}")
                }
                }
            }
        }catch (e:Exception){
    ...
    fun genratePaymentPageURL(context: Context, queryParam: String): String =
        runBlocking(Dispatchers.IO) {
            ...
            if (queryParam.contains("plan_id")) {
                ...
                url = "https://chpayments.hungama.com/plan?"
            }
            ...
            url += "&appsflyer_id=${AppsFlyerLib.getInstance().getAppsFlyerUID(context)}"
    ...
    fun getAllUpiEnabledAppList(context: Context): ArrayList<String> = runBlocking(Dispatchers.IO) {
        ...
        try {
            ...
            if (paymentApps.isEmpty())
                setPayAppName()
            paymentApps.forEach{
                setLog("paymentKeyValue", "${it.key} ${it.value}" )

                if (!it.value.contains(upilist.toString()))
                    if (isAppInstalled(context, it.key))
                    upilist.add(it.value)
            }
...
//            context.packageManager.getPackageInfo(packageName, PackageManager.GET_META_DATA)
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.TIRAMISU) {
                ...
                @Suppress("DEPRECATION") context.packageManager.getPackageInfo(packageName, PackageManager.GET_META_DATA)
            }
            true
        } catch (e: PackageManager.NameNotFoundException) {
            false
    ...
    fun PackageManager.isAppInstalled(packageName: String, flags: Int = 0): PackageInfo =
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.TIRAMISU) {
    ...
    fun setPayAppName(){
        paymentApps.put("com.google.android.apps.nbu.paisa.user", "Gpay")
        paymentApps.put("net.one97.paytm", "Paytm")
        paymentApps.put("com.phonepe.app", "PhonePe")
        paymentApps.put("in.amazon.mShop.android.shopping", "AmazonPay")
        paymentApps.put("com.mobikwik_new", "MobiKwik")
        paymentApps.put("com.freecharge.android", "Freecharge")
        paymentApps.put("com.dreamplug.androidapp", "cred")
    }
    ...
    public suspend fun setDownloadEventModelDataAppLevel(selectedContentId: String) {
        ...
        if (downloadedAudio != null) {
            ...
            if (TextUtils.isEmpty(eventModel?.episodeNumber)) {
                eventModel?.episodeNumber = "0"
            }
    ...
    fun String?.checkNull(): String? {
          var str=""
        if(String.toString().isNullOrEmpty() || String.toString().isNullOrBlank()){
    ...
    fun getbanner(context: Context, iv_banner: ImageView, trigger: String) {
        if (context != null && iv_banner != null) {
            ...
            if (!isUserHasGoldSubscription()) {
                ...
                if (getFirebaseConfigAdsData().enablePaymentNudge && Constant.DEFAULT_COUNTRY_CODE.equals("IN", true)) {
                    iv_banner.show()
                }
    ...
    fun setButtonFromFirebase(
     ...
     fun setGoldButton(context:Context,dash_gold:ImageView){
        var ft: AdsConfigModel.Ft? = AdsConfigModel.Ft()
        var nonft: AdsConfigModel.Nonft? = AdsConfigModel.Nonft()
         ...
         MainScope().async {
             if(!isUserHasGoldSubscription()) {
                 ...
                 dash_gold.show()
                 if (isUserHasEliableFreeContent()) {
                     ft = getFirebaseConfigAdsData().nudgeHomeHeaderBanner.ft
                     ...
                     nonft = CommonUtils.getFirebaseConfigAdsData().nudgeHomeHeaderBanner.nonft
    ...
    fun checkMovieRight(movierights: List<String>):Boolean {
    ...
    fun checkMovieRight(movierights: String):Boolean {
        return if(movierights.contains("AMOD")) true else false
    }

    fun checkGoldMusic(movierights:String,contentTypeId:Int):Boolean {
        if (contentTypeId.equals("110") || contentTypeId.equals("109")) {
            return !checkGoldMusicPodcast()
        }else {
            if (!isUserHasGoldSubscription() && !movierights.contains("AMOD")) {
                return true
            }
        }
         return false
    ...
    fun checkGoldMusicPodcast():Boolean{

        if (isUserHasGoldSubscription()) return true

        if (!isUserHasGoldSubscription() && !BaseActivity.songDataList.isNullOrEmpty()
            && BaseActivity.songDataList.size > BaseActivity.nowPlayingCurrentIndex()) {

                val list_movierights: ArrayList<String> = ArrayList<String>()
                list_movierights.add(BaseActivity.songDataList.get(BaseActivity.nowPlayingCurrentIndex()).movierights)

                   return !checkMovieRight(list_movierights)        ///check movie rights for podcast AMOD
            }
       return false
    }

    fun checkEnableFlag(drawer: AdsConfigModel.DrawerDownloadAll,movierights:String=""):String {
        ...
        if (isUserHasEliableFreeContent())
            ft = drawer.ft
        else nonft = drawer.nonft

        if (movierights.contains(PlanNames.SVOD.name, true) || movierights.isEmpty() ){
            if (isUserHasEliableFreeContent()) {
                if (ft != null) {
                    return drawer.enable_drawer
                } else ft = null
    ...
    fun checkProdcastCheck(context: Context, movieR: String, imageView: ImageView, size: Int = 0) {
        val firebaseConfigAdsData = getFirebaseConfigAdsData()
        val ribbonGold = firebaseConfigAdsData.ribbon_pdc_gold
        val ribbonFree = firebaseConfigAdsData.ribbon_pdc_free
        val ribbonFreeEpisodes = firebaseConfigAdsData.ribbon_pdc_limited
        imageView.hide()
        val movieRight = movieR.replace("[", "").replace("]", "")

        if (movieRight.isNotBlank()) {
            val showImage = when {
                movieRight.contains("SMOD") && movieRight.contains("AMOD") && ribbonFreeEpisodes -> {
                    R.drawable.free_episode
                }
                movieRight.contains("SMOD") && ribbonGold -> {
                    R.drawable.image_gold_ribbon_strip
                }
                movieRight.contains("AMOD") && ribbonFree -> {
                    R.drawable.image_free_ribbon_strip
                }
                else -> null
            }
    ...
    fun setMargin(rlChildMain:RelativeLayout, left:Int){
        val param = rlChildMain.layoutParams as ViewGroup.MarginLayoutParams
        param.setMargins(left,0,0,0)
        rlChildMain.layoutParams = param
    }

    fun checkAudioBookCheck(context: Context, movieR: String, imageView: ImageView, llMain: RelativeLayout?) {
        val firebaseConfigAdsData = getFirebaseConfigAdsData()
        val ribbonGold = firebaseConfigAdsData.ribbon_audio_book_gold
        val ribbonFree = firebaseConfigAdsData.ribbon_audio_book_free
        val ribbonFreeEpisodes = firebaseConfigAdsData.ribbon_audio_book_limited
        imageView.hide()
        val movieRight = movieR.replace("[", "").replace("]", "")

        if (movieRight.isNotBlank()) {
            var showImage: Int? = null/*when {
                movieRight.contains("SMOD") && movieRight.contains("AMOD") && ribbonFreeEpisodes -> {
                    R.drawable.free_episode
                }
                movieRight.contains("SMOD") && ribbonGold -> {
            ...
            if(movieRight.contains("SMOD") && movieRight.contains("AMOD")) {
                if (ribbonFreeEpisodes)
                {
                    showImage = R.drawable.free_episode
                    setImage(showImage, context, imageView, llMain)
                }

                return
            }
            if(movieRight.contains("SMOD")) {
                if (ribbonGold)
                {
                    showImage = R.drawable.image_gold_ribbon_strip
                    setImage(showImage, context, imageView, llMain)
                }
            ...
            if(movieRight.contains("AMOD")) {
                if (ribbonFree)
                {
                    showImage = R.drawable.image_free_ribbon_strip
                    setImage(showImage, context, imageView, llMain)
                }
                return
            }
    ...
    fun setImage(showImage:Int, context: Context, imageView: ImageView, llMain: RelativeLayout?){
        showImage?.let {
            imageView.apply {
                show()
                background = ContextCompat.getDrawable(context, it)
                if(llMain != null){
                    val layoutParams = llMain.layoutParams
                    val params = layoutParams as ViewGroup.MarginLayoutParams
                    params.setMargins(context.resources.getDimensionPixelSize(R.dimen.dimen_9)
                        , context.resources.getDimensionPixelSize(R.dimen.dimen_10),
                        context.resources.getDimensionPixelSize(R.dimen.dimen_16),
                        0)
    ...
    fun checkAudioStoriesCheck(context: Context, movieR: String, imageView: ImageView, llMain: RelativeLayout?) {
        val firebaseConfigAdsData = getFirebaseConfigAdsData()
        val ribbonGold = firebaseConfigAdsData.ribbon_audio_stories_gold
        val ribbonFree = firebaseConfigAdsData.ribbon_audio_stories_free
        val ribbonFreeEpisodes = firebaseConfigAdsData.ribbon_audio_stories_limited
        imageView.hide()
        val movieRight = movieR.replace("[", "").replace("]", "")

        if (movieRight.isNotBlank()) {
            var showImage : Int? = null
            /* when {
                movieRight.contains("SMOD") && movieRight.contains("AMOD") && ribbonFreeEpisodes -> {
                    R.drawable.free_episode
                }
...
/*            showImage?.let {
                imageView.apply {
                    ...
                    if(llMain != null) {
                        ...
                        val params = layoutParams as ViewGroup.MarginLayoutParams
                        params.setMargins(
                            context.resources.getDimensionPixelSize(R.dimen.dimen_9),
                            context.resources.getDimensionPixelSize(R.dimen.dimen_16),
                            context.resources.getDimensionPixelSize(R.dimen.dimen_16),
                            0
                        )
